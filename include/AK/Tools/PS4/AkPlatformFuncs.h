...
namespace AKPLATFORM
{
	...
	AkForceInline void* AllocVM(size_t size, size_t* extra)
	{
		...
		int32_t err;
		err = sceKernelAllocateDirectMemory(0, SCE_KERNEL_MAIN_DMEM_SIZE, size, AK_VM_PAGE_SIZE, SCE_KERNEL_WB_ONION, &directMemStart);
		if (err == SCE_OK)
		{
			err = sceKernelMapDirectMemory(&ptr, size, SCE_KERNEL_PROT_CPU_RW, 0, directMemStart, AK_VM_PAGE_SIZE);
			AKASSERT(ptr);
			AKASSERT(err == SCE_OK);
		...
		const char * in_szThreadName )				// Opt thread name.
    {
		...
		AK::MemoryMgr::InitForThread( in_szThreadName );
    }
