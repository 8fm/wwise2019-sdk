...
struct AkPlatformInitSettings
{
	...
	AkUInt32			uSampleRate;				///< Sampling Rate. Default 48000 Hz, the recommended sampling rate on Stadia.
	...
	AkAudioAPI			eAudioAPI;					///< Main audio API to use. Leave to AkAPI_Default for the default sink (default value).
														...
														///< \ref AkAudioAPI
	AkDataTypeID		sampleType;					///< Sample type. AK_FLOAT for 32 bit float, AK_INT for 16 bit signed integer, defaults to AK_INT.
													///< AK_INT is the recommended sample type for PulseAudio on Stadia (will use PA_SAMPLE_S16LE).
...
namespace AK
{
	...
	{
		/// Finds the device ID for a particular ggp keyboard struct.
		/// \return The device ID.
		AK_EXTERNAPIFUNC( AkUInt32, GetDeviceID ) (const ggp::Keyboard& in_keyboard);

		/// Finds the device ID for a particular ggp gamepad struct.
		/// \return The device ID.
		AK_EXTERNAPIFUNC( AkUInt32, GetDeviceID ) (const ggp::Gamepad& in_gamepad);
	}
}
