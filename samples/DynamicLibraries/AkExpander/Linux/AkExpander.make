...
ifeq ($(config),debug)
  PREMAKE4_BUILDTARGET_BASENAME = AkExpander
  TARGETDIR = ../../../../Linux_$(AK_LINUX_ARCH)/Debug/bin
  TARGET = $(TARGETDIR)/libAkExpander.so
  ...
  LIBS += -lAkExpanderFX
  LDDEPS +=
  ALL_LDFLAGS += $(LDFLAGS) -L"../../../../Linux_$(AK_LINUX_ARCH)/Debug/lib" -shared -Wl,-soname=libAkExpander.so -Wl,--export-dynamic
...
ifeq ($(config),profile)
  ...
  LIBS += -lAkExpanderFX
  LDDEPS +=
  ALL_LDFLAGS += $(LDFLAGS) -L"../../../../Linux_$(AK_LINUX_ARCH)/Profile/lib" -shared -Wl,-soname=libAkExpander.so -Wl,--export-dynamic
  LINKCMD = $(CXX) -o "$@" $(OBJECTS) $(RESOURCES) $(ALL_LDFLAGS) $(LIBS)
  define PREBUILDCMDS
  endef
  define PRELINKCMDS
...
ifeq ($(config),release)
  ...
  LIBS += -lAkExpanderFX
  LDDEPS +=
  ALL_LDFLAGS += $(LDFLAGS) -L"../../../../Linux_$(AK_LINUX_ARCH)/Release/lib" -shared -Wl,-soname=libAkExpander.so -Wl,--export-dynamic
  LINKCMD = $(CXX) -o "$@" $(OBJECTS) $(RESOURCES) $(ALL_LDFLAGS) $(LIBS)
  define PREBUILDCMDS
  endef
  define PRELINKCMDS
