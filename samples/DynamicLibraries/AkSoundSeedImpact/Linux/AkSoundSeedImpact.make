...
ifeq ($(config),debug)
  PREMAKE4_BUILDTARGET_BASENAME = AkSoundSeedImpact
  TARGETDIR = ../../../../Linux_$(AK_LINUX_ARCH)/Debug/bin
  TARGET = $(TARGETDIR)/libAkSoundSeedImpact.so
  ...
  LIBS += -lAkSoundSeedImpactFX
  LDDEPS +=
  ALL_LDFLAGS += $(LDFLAGS) -L"../../../../Linux_$(AK_LINUX_ARCH)/Debug/lib" -shared -Wl,-soname=libAkSoundSeedImpact.so -Wl,--export-dynamic
...
ifeq ($(config),profile)
  ...
  LIBS += -lAkSoundSeedImpactFX
  LDDEPS +=
  ALL_LDFLAGS += $(LDFLAGS) -L"../../../../Linux_$(AK_LINUX_ARCH)/Profile/lib" -shared -Wl,-soname=libAkSoundSeedImpact.so -Wl,--export-dynamic
  LINKCMD = $(CXX) -o "$@" $(OBJECTS) $(RESOURCES) $(ALL_LDFLAGS) $(LIBS)
  define PREBUILDCMDS
  endef
  define PRELINKCMDS
...
ifeq ($(config),release)
  ...
  LIBS += -lAkSoundSeedImpactFX
  LDDEPS +=
  ALL_LDFLAGS += $(LDFLAGS) -L"../../../../Linux_$(AK_LINUX_ARCH)/Release/lib" -shared -Wl,-soname=libAkSoundSeedImpact.so -Wl,--export-dynamic
  LINKCMD = $(CXX) -o "$@" $(OBJECTS) $(RESOURCES) $(ALL_LDFLAGS) $(LIBS)
  define PREBUILDCMDS
  endef
  define PRELINKCMDS
...
$(TARGET): $(GCH) ${CUSTOMFILES} $(OBJECTS) $(LDDEPS) $(RESOURCES) | $(TARGETDIR)
	@echo Linking AkSoundSeedImpact
	$(SILENT) $(LINKCMD)
	$(POSTBUILDCMDS)
...
clean:
	@echo Cleaning AkSoundSeedImpact
ifeq (posix,$(SHELLTYPE))
	$(SILENT) rm -f  $(TARGET)
	$(SILENT) rm -rf $(OBJDIR)
else
