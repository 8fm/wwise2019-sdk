...
wchar_t * SFReadBytesMem::ReadAndCopyString()
{
	long cChars;
	if ( !Read<long>( cChars ) ) 
		return NULL;

	wchar_t* szStr = (wchar_t*)malloc((cChars+1)*sizeof(wchar_t));

	long cRead;
	ReadBytes(szStr, cChars * sizeof(wchar_t), cRead);
	szStr[cChars] = 0;

	if (cRead/sizeof(wchar_t) != cChars)
	{
		//Error
		free(szStr);
		return NULL;
