...
bool CSoundFrame::SeekOnEvent(AkUniqueID in_eventID, AkGameObjectID in_gameObjectID, AkTimeMs in_iPosition, bool in_bSeekToNearestMarker, AkPlayingID in_PlayingID)
{
	if (!m_hwndServer)
		return false;
	...
	bytes.Write<AkUniqueID>(in_eventID);
	WriteGameObjectID(bytes, in_gameObjectID);
	bytes.Write<AkTimeMs>(in_iPosition);
	bytes.Write<bool>(in_bSeekToNearestMarker);
	bytes.Write<AkPlayingID>(in_PlayingID);

	SendCopyData(SF_COPYDATA_SEEKONEVENT_MS, bytes);

	return true;
}

bool CSoundFrame::SeekOnEvent(AkUniqueID in_eventID, AkGameObjectID in_gameObjectID, AkReal32 in_fPercent, bool in_bSeekToNearestMarker, AkPlayingID in_PlayingID)
{
	if (!m_hwndServer)
		return false;
	...
	bytes.Write<AkUniqueID>(in_eventID);
	WriteGameObjectID(bytes, in_gameObjectID);
	bytes.Write<AkReal32>(in_fPercent);
	bytes.Write<bool>(in_bSeekToNearestMarker);
	bytes.Write<AkPlayingID>(in_PlayingID);

	SendCopyData(SF_COPYDATA_SEEKONEVENT_PCT, bytes);
...
bool CSoundFrame::SetListeners( AkGameObjectID in_emitterGameObj, const AkGameObjectID* in_pListenerGameObjs, AkUInt32 in_uNumListeners )
{
	if ( !m_hwndServer )
		return false;

	SFWriteBytesMem bytes;
	WriteGameObjectID( bytes, in_emitterGameObj);
	bytes.Write<AkUInt32>( in_uNumListeners );

	if (in_uNumListeners)
	{
		for (AkUInt16 i(0); i < in_uNumListeners; ++i)
		{
			bytes.Write<AkUInt64>( in_pListenerGameObjs[i] );
		}
	}
...
bool CSoundFrame::SetListenerSpatialization( AkUInt32 in_uiIndex, bool in_bSpatialized, AkChannelConfig in_channelConfig, AkReal32 * in_pVolumeOffsets /*= NULL*/ )
{
	if ( !m_hwndServer )
		return false;
	...
	bytes.Write<AkChannelConfig>( in_channelConfig );

	if( in_pVolumeOffsets )
	{
		for( unsigned int i = 0; i < in_channelConfig.uNumChannels; ++i )
			bytes.Write<AkReal32>( in_pVolumeOffsets[i] );
	}
...
bool CSoundFrame::SetGameObjectOutputBusVolume(AkGameObjectID in_gameObjectID, AkGameObjectID in_listenerID, AkReal32 in_fControlValue )
{
	if ( !m_hwndServer )
		return false;
	...
	SFWriteBytesMem bytes;
	WriteGameObjectID( bytes, in_gameObjectID );
	WriteGameObjectID( bytes, in_listenerID );
...
bool CSoundFrame::ProcessSoundBankDnD( IDataObject * in_pDataObject, ISoundBankList ** out_ppSoundBankList )
{
	return ProcessObjectDnD( in_pDataObject, out_ppSoundBankList, SFType_SoundBank );
}
...
bool CSoundFrame::GetSoundBank( AkUniqueID in_SoundBankID, ISoundBank ** out_ppArgument ) const
{
	return GetObject<ISoundBank, ISoundBankList>( in_SoundBankID, out_ppArgument , SFType_Argument );
}

bool CSoundFrame::GetSoundBankList( ISoundBankList ** out_ppSoundBankList ) const
{
	return GetObjectList( out_ppSoundBankList, WM_SF_REQSOUNDBANKS );
}
...
bool CSoundFrame::GetSoundBankGenerationSettings(
	bool in_bUser,
	SoundBankGenerationSettings & out_settings
	)
{
	if (!m_hwndServer)
		return false;
	...
	SFWriteBytesMem bytes;
	bytes.Write<bool>(in_bUser);
	SendCopyData(SF_COPYDATA_GETSOUNDBANKGENERATIONSETTINGS, bytes);

	if (m_pCopyData)
	{
		out_settings = *((SoundBankGenerationSettings*)m_pCopyData);
...
bool CSoundFrame::GenerateSoundBanks(
	LPCWSTR * in_pszBanks, long in_nBanks,
	LPCWSTR * in_pszPlatforms, long in_nPlatforms,
	LPCWSTR * in_pszLanguages, long in_nLanguages,
	SoundBankGenerationSettings * in_pSettings)
{
	if ( !m_hwndServer )
		return false;
	...
	bytes.Write<bool>(in_pSettings != NULL);
	if (in_pSettings)
	{
		SoundBankGenerationSettings_Serialize(in_pSettings, &bytes);
	}
	...
	bool bResult = (AkUIntPtr) m_pCopyData != 1;
	m_pCopyData = NULL;
	...
	case SF_COPYDATA_OBJECTLIST:
		{
			...
			case SFType_ConversionSettings:
				...
				break;
			case SFType_SoundBank:
				m_pCopyData = GetCopyDataObjectList<SFSoundBank, SoundBankList>( bytes );
				break;
			default:
	...
	case SF_COPYDATA_NOTIF:
		{
			...
			case SFType_SoundBank:
				m_pClient->OnSoundBankNotif( (IClient::Notif) notifData.eNotif, (AkUniqueID)notifData.objectID );
				break;
	...
	case SF_COPYDATA_GENERATESOUNDBANKSRESULT:
		{
			...
			m_pCopyData = (void *) (AkUIntPtr) result;
		}
		break;

	case SF_COPYDATA_SOUNDBANKGENERATIONSETTINGS:
	{
		SFReadBytesMem bytes(pCopyData->lpData, pCopyData->cbData);

		m_pCopyData = new SoundBankGenerationSettings;

		SoundBankGenerationSettings_Deserialize(static_cast<SoundBankGenerationSettings*>(m_pCopyData), &bytes);
	}
	break;
...
{
	in_rWriter.Write<AkGameObjectID>(in_id);
}

bool CSoundFrame::ShowWwiseObject( const GUID* in_pGuids, long in_cGuids, ShowLocation in_eLocation )
{
	if ( in_pGuids == NULL || !m_hwndServer || in_cGuids <= 0)
		return false;

	SFWriteBytesMem bytes;

	bytes.Write<long>( in_eLocation );
	bytes.Write<long>( in_cGuids );

	for( int i = 0; i < in_cGuids; ++i )
		bytes.Write<GUID>( in_pGuids[i] );

	SendCopyData( SF_COPYDATA_SHOWOBJECTS, bytes );
...
void * CSoundFrame::GetCopyData()
{
	return m_pCopyData;
}

void CSoundFrame::SetCopyData(void *p)
{
	m_pCopyData = p;
}
