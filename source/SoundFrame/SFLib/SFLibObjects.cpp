...
SFSoundBank * SFSoundBank::From( IReadBytes * in_pBytes )
{
	SFSoundBank * pObj = new SFSoundBank();
...
ISoundBank * ISoundBank::From( IReadBytes * in_pBytes )
{
	return SFSoundBank::From( in_pBytes );
}
	
bool SFSoundBank::To( IWriteBytes * io_pBytes )
{
	bool bSuccess = WriteIDs( io_pBytes )
		&& io_pBytes->WriteString( m_wszName );
