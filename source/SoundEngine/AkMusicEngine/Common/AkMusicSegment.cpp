...
{
	CAkMusicSegment * pSegment = AkNew(AkMemID_Structure, CAkMusicSegment( in_ulID ) );
    if( pSegment )
	{
...
{
	AKOBJECT_TYPECHECK(AkNodeCategory_MusicSegment);
	...
	if( ulNumMarkers )
	{
		AkMusicMarkerWwise* pArrayMarkers = (AkMusicMarkerWwise*)AkMalign(AkMemID_Structure, ulNumMarkers*sizeof( AkMusicMarkerWwise ), AK_64B_OS_STRUCT_ALIGN );
		if( !pArrayMarkers )
		...
		for( AkUInt32 i = 0; i < ulNumMarkers; ++i )
		{
			...
			if ( uStringSize > 0 )
			{
				pArrayMarkers[i].pszName = (char*)AkAlloc(AkMemID_Structure, uStringSize + 1 );
		...
		if (eResult != AK_Success)
		{
			...
			{
				if (pArrayMarkers[i].pszName)
					AkFree(AkMemID_Structure, pArrayMarkers[i].pszName);
			}
		}
		AkFalign(AkMemID_Structure, pArrayMarkers );
	}
...
void CAkMusicSegment::Seek( ActionParams& in_rAction )
{
	...
	if ( rSeekActionParams.bIsSeekRelativeToDuration )
	{
		...
		CAkMusicRenderer::SeekPercent( this, rSeekActionParams.pGameObj, rSeekActionParams.playingID, fSeekPercent, rSeekActionParams.bSnapToNearestMarker );
	}
	else
	{
...
void CAkMusicSegment::ExecuteAction( ActionParams& in_rAction )
{
...
void CAkMusicSegment::ExecuteActionExcept( ActionParamsExcept& in_rAction )
{
	if ( ActionParamType_Seek == in_rAction.eType )
	{
		Seek( in_rAction );
	...
	{
		CAkMusicNode::ExecuteActionExcept( in_rAction );
	}
}
...
{
    return CreateSegmentContext( in_pParentCtx, in_GameObject, in_rUserparams, in_bPlayDirectly );
}
    ...
    )
{
    ...
    CAkSequencableSegmentCtx * pCtx = AkNew(AkMemID_Object, CAkSequencableSegmentCtx(
        this,
        in_pParentCtx ) );
    if ( pCtx )
    {
		pCtx->AddRef();
        if ( ( pCtx->Init( in_GameObject, in_rUserparams, in_bPlayDirectly ) == AK_Success ) && ( pCtx->RefCount() > 1 ) )
		{
			pCtx->Release();
		}
		else
    ...
    )
{
    ...
    CAkSegmentCtx * pSegmentCtx = AkNew(AkMemID_Object, CAkSegmentCtx(
        this,
        in_pParentCtx ) );
    if ( pSegmentCtx )
    {
		pSegmentCtx->AddRef();
        if ( pSegmentCtx->Init( in_GameObject, in_rUserparams, in_bPlayDirectly ) != AK_Success )
        {
            pSegmentCtx->_Cancel();
			pSegmentCtx->Release();
            pSegmentCtx = NULL;
...
AKRESULT CAkMusicSegment::PlayInternal( AkPBIParams& in_rPBIParams )
{
    ...
    CAkChainCtx * pCtx = CreateSegmentContext( NULL, in_rPBIParams.pGameObj, in_rPBIParams.userParams, in_rPBIParams.bPlayDirectly );
    if ( pCtx )
    {
...
{
	out_uRequiredEntryPosition = in_uMinimumEntryPosition;
    ...
    if ( in_rule.eEntryType == EntryTypeEntryMarker )
	{
		...
		/** REVIEW: This is the true implementation as it enforces synching in on Entry Cue, but it makes custom seeking in switch containers unusable.
		else
			out_uRequiredEntryPosition = ActiveDuration();
		**/
	}
...
void CAkMusicSegment::NotifyUserCuesInRange(
	...
	const AkSegmentInfo & in_segmentInfo, // Segment info to pass to notification
	...
	)
{
	...
	while ( uIndex <= uLastIndex )
    {
        if ( m_markers[uIndex].uPosition >= uStartPosition )
        {
			...
			g_pPlayingMgr->NotifyMusicUserCues(in_playingID, in_segmentInfo, m_markers[uIndex].GetName());
        }
        ++uIndex;
    }
}
