...
,m_bWasPlaybackSkipped( false )
{
    if ( m_pSegment )
	{
		...
		m_segmentID = m_pSegment->SegmentNode()->ID();
	}
	else
		m_segmentID = AK_INVALID_UNIQUE_ID;
}
    ...
    )
{
	...
	if ( m_pSegment )
	{
		...
		if ( m_pSegment && m_pSegment->RequiresProcessing() )
			m_pSegment->Process( iShortRangeCurTime, in_uNumSamples, in_fPlaybackSpeed );
	}
    ...
    while ( itAction != m_listAssociatedActions.End() )
    {
		...
		if ( iCurrentFrameEnd > pAction->iRelativeTime )
		{
            ...
            case AssocActionTypeState:
                ...
                CAkMusicRenderer::PerformDelayedStateChange( pAction->pStateChangeCookie );
                break;
            }
...
AKRESULT CAkScheduledItem::GetInfo(
	...
	AkReal32		in_fPlaybackSpeed		// Playback speed factor
	) const
{
	if ( m_pSegment )
	{
		...
		AkReal32 fOneOverRate = 1.f / (in_fPlaybackSpeed * AK_CORE_SAMPLERATE);
		out_uSegmentInfo.fBarDuration = (((AkReal32)rGrid.uBarDuration) * fOneOverRate);
		out_uSegmentInfo.fBeatDuration = (((AkReal32)rGrid.uBeatDuration) * fOneOverRate);
		out_uSegmentInfo.fGridDuration = (((AkReal32)rGrid.uGridDuration) * fOneOverRate);
		out_uSegmentInfo.fGridOffset = (((AkReal32)rGrid.uGridOffset) * fOneOverRate);
	}
	else
	{
		...
		out_uSegmentInfo.iRemainingLookAheadTime = 0;
		out_uSegmentInfo.fBarDuration = 0.f;
		out_uSegmentInfo.fBeatDuration = 0.f;
		out_uSegmentInfo.fGridDuration = 0.f;
		out_uSegmentInfo.fGridOffset = 0.f;
	}	
	return AK_Success;
}
	...
	)
{
	...
	TransParams revTransParams(AkTimeConv::SamplesToMilliseconds( iTimeElapsed ));
	...
	) const
{
	...
	GetInfo(in_iSegmentPosition, segmentInfo, in_fPlaybackSpeed);
	...
	if( in_uNotifFlags & AK_MusicSyncEntry )
	{
		...
		{
			g_pPlayingMgr->NotifyMusic(in_playingID, AK_MusicSyncEntry, segmentInfo);
		}
	}
	...
	if( in_uNotifFlags & AK_MusicSyncBar )
	{
		for ( AkUInt32 uBar = 0; uBar<uNumBars; uBar++ )
			g_pPlayingMgr->NotifyMusic(in_playingID, AK_MusicSyncBar, segmentInfo);
	}
	...
	if( in_uNotifFlags & AK_MusicSyncBeat )
	{
		for ( AkUInt32 uBeat = 0; uBeat<uNumBeats; uBeat++ )
			g_pPlayingMgr->NotifyMusic(in_playingID, AK_MusicSyncBeat, segmentInfo);
	}
	...
	if( in_uNotifFlags & AK_MusicSyncGrid )
	{
		for ( AkUInt32 uGrid = 0; uGrid<uNumGrids; uGrid++ )
			g_pPlayingMgr->NotifyMusic(in_playingID, AK_MusicSyncGrid, segmentInfo);
	}
	...
	if( in_uNotifFlags & AK_MusicSyncExit )
	{
		...
		{
			g_pPlayingMgr->NotifyMusic(in_playingID, AK_MusicSyncExit, segmentInfo);
		}
	}
}
...
CAkMusicNode * CAkMatrixSequencer::GetActiveNode()
{
	...
	CAkMusicNode * pNode = ( pSegmentNode ) ? pSegmentNode : pParentNode;
...
void CAkMatrixSequencer::Execute( AkUInt32 in_uNumFrames )
{
    ...
    {
		CAkMusicNode * pActiveNode = GetActiveNode();

		if ( !m_bParametersValid && pActiveNode )
			RefreshParameters( in_uNumFrames, pActiveNode );

		in_uNumFrames = AkMath::Round( in_uNumFrames * GetPlaybackSpeed() );
		...
		// Process MIDI clips for next audio frame.
		GetMidiClipMgr()->NextFrame( in_uNumFrames, GetPlaybackSpeed() );
		...
		// Invalidate parameters for next frame if active node has changed during this frame.
		if ( GetActiveNode() != pActiveNode )
			m_bParametersValid = false;
    }
	...
	)
{
    ...
    while ( itAction != in_listActions.End() )
    {
        ...
        case AssocActionTypeState:
            ...
            CAkMusicRenderer::RescheduleDelayedStateChange( pAction->pStateChangeCookie );
            break;
...
void CAkMatrixSequencer::RefreshParameters(
	AkUInt32		in_uWindowSize,		// Window size, used for adjusting speed.
	CAkMusicNode *	in_pActiveNode		// Active segment node (or parent structure if <nothing>)
	)
{
	AKASSERT( in_pActiveNode );

	AkReal32 fOldPlaybackSpeed = m_musicParams.fPlaybackSpeed;

	if( m_bLiveEdit )
		m_pOwner->_TriggerModulators();

	m_musicParams.Clear();
	AkRTPCKey rtpcKey;
	rtpcKey.GameObj() = GameObjectPtr();
	rtpcKey.PlayingID() = m_UserParams.PlayingID();
	in_pActiveNode->GetMusicParams( m_musicParams, rtpcKey );

	// Clamp playback speed. Cannot support greater ratios because pitch shift does not...
	AkReal32 fNewPlaybackSpeed = AkClamp( m_musicParams.fPlaybackSpeed, AK_MIN_PLAYBACK_SPEED, AK_MAX_PLAYBACK_SPEED );

	// Adjust speed so that it can be implemented with an integer window size to avoid drifting.
	AkUInt32 uNewWindowSize = AkMath::Round( fNewPlaybackSpeed * in_uWindowSize );
	fNewPlaybackSpeed = (AkReal32)uNewWindowSize / (AkReal32)in_uWindowSize;

	m_musicParams.fPlaybackSpeed = fNewPlaybackSpeed;

	// Notification can be costly so we rather check that it really changed.
	if ( ( fNewPlaybackSpeed - fOldPlaybackSpeed ) != 0 )
		m_pOwner->_NotifyPropertyChange( AkPropID_PlaybackSpeed, fNewPlaybackSpeed );
...
{
	// Ensure owner is not stopping
	if ( m_pOwner->IsStopping() )
		return;
    ...
    while ( eSyncFoundResult != AK_Success && pStingerData && window.IsValid() );
}
    ...
    )
{
	...
	AkStingerRecord * pStingerRecord = AkNew(AkMemID_Object, AkStingerRecord );
	if ( !pStingerRecord )
		return AK_Fail;

	AkAssociatedAction * pAction = AkNew(AkMemID_Object, AkAssociatedAction );
	if ( !pAction )
	{
		AkDelete(AkMemID_Object, pStingerRecord );
		return AK_Fail;
	}
    ...
    )
{
    ...
    if ( pStingerNode )
    {
		...
		CAkMatrixAwareCtx * pStingerCtx = pStingerNode->CreateContext( m_pOwner, m_pGameObj, m_UserParams, false );
        if ( pStingerCtx )
        {
			...
			if ( pStingerCtx->Prepare( &ruleEntry, NULL, seekInfo, out_iLookAheadDuration, uSelectedCue ) == CAkMatrixAwareCtx::CtxPrepare_Failure )
			{
				pStingerCtx->_Cancel();
				pStingerCtx = NULL;
			}
	...
	while ( it != m_listPendingStingers.End() )
	{
		...
		else
		{
			...
			if ( in_iCurrentTime > pRecord->AbsoluteDontRepeatTime() )
			{
				...
				AkDelete(AkMemID_Object, pRecord );
			}
			else
			{
    ...
    )
{
	...
	if ( !window.IsValid() )
	{
		out_bStateGroupUsed = false;
		return 0;
	}

	// Assume the state group is used
	out_bStateGroupUsed = true;
	...
	if ( !window.IsDurationInfinite() )
	{
		...
		if ( !pNextCtx )
		{
			...
			out_bStateGroupUsed = AK_DelayedStateNotHandled != eFirstSegmentResult;
			 ...
			 pNextCtx->IsPlaying() )
		{
			...
			AkDelayedStateHandlingResult eHandlingResult = GetEarliestStateSyncTime( window, 
																					 in_stateGroupID, 
																					 out_iSyncTime );
			// Assume that the state group is used
			out_bStateGroupUsed = (AK_DelayedStateNotHandled != eHandlingResult
								|| AK_DelayedStateNotHandled != eFirstSegmentResult);
    ...
    )
{
    ...
    // Create action.
    AkAssociatedAction * pStateChangeAction = AkNew(AkMemID_Object, AkAssociatedAction );
    ...
    )
{
    ...
    CAkStateAware::CAkStateSyncArray stateSyncs;
	...
	)
{
	...
	do
	{
		window.NotifyMusicCallbacks( in_iCurrentTime, in_uFrameDuration, uMusicSyncFlags, PlayingID(), GetPlaybackSpeed() );
		if ( window.IsDurationInfinite() 
