...
{
	CAkMusicRanSeqCntr * pSequence = AkNew(AkMemID_Structure, CAkMusicRanSeqCntr(in_ulID));
    if( pSequence )
	{
...
{
	AKOBJECT_TYPECHECK(AkNodeCategory_MusicRanSeqCntr);		
	...
	if( numPlaylistItems )
	{
		AkMusicRanSeqPlaylistItem* pPlayList = (AkMusicRanSeqPlaylistItem*)AkAlloc(AkMemID_Object, numPlaylistItems*sizeof(AkMusicRanSeqPlaylistItem) );
		if( !pPlayList ) 
			return AK_Fail;
		for( AkUInt32 i = 0; i < numPlaylistItems; ++i )
		{
			...
			pPlayList[i].m_LoopMin			= READBANKDATA( AkInt16, in_pData, in_ulDataSize );
			pPlayList[i].m_LoopMax			= READBANKDATA( AkInt16, in_pData, in_ulDataSize );
		...
		AkFree(AkMemID_Object, pPlayList );
	}
...
void CAkMusicRanSeqCntr::FlushPlaylist()
{
	...
	for ( RSInfoMap::Iterator it = m_globalRSInfo.Begin(); it != m_globalRSInfo.End(); ++it )
	{
...
void CAkMusicRanSeqCntr::ExecuteAction( ActionParams& in_rAction )
{
    if ( ActionParamType_Seek == in_rAction.eType )
	{
		...
		if ( SeekTypeTimeAbsolute == rSeekActionParams.eSeekType )
		{
			...
			if ( iSeekTime < 0 )
				iSeekTime = 0;
			CAkMusicRenderer::SeekTimeAbsolute( this, rSeekActionParams.pGameObj, iSeekTime, rSeekActionParams.bSnapToNearestMarker );
		}
		else if ( SeekTypePercent == rSeekActionParams.eSeekType )
		{
			...
			CAkMusicRenderer::SeekPercent( this, rSeekActionParams.pGameObj, fSeekPercent, rSeekActionParams.bSnapToNearestMarker );
		}
		else 
...
CAkMatrixAwareCtx * CAkMusicRanSeqCntr::CreateContext( 
	...
	bool in_bPlayDirectly /* = false */
    )
{
    return CreateSequenceCtx( in_pParentCtx, in_GameObject, in_rUserparams, in_bPlayDirectly );
}
...
{
    CAkSequenceCtx * pCtx = AkNew(AkMemID_Object, CAkSequenceCtx(
        this,
        in_pParentCtx ) );
    if ( pCtx )
    {
...
AKRESULT CAkMusicRanSeqCntr::PlayInternal( AkPBIParams& in_rPBIParams )
{
    ...
    CAkSequenceCtx * pCtx = CreateSequenceCtx( NULL, in_rPBIParams.pGameObj, in_rPBIParams.userParams, in_rPBIParams.bPlayDirectly );
    if ( pCtx )
    {
...
bool CAkMusicRanSeqCntr::CheckPlaylistHasChanged( AkMusicRanSeqPlaylistItem* in_pArrayItems )
{
    ...
    if( 
		...
		|| m_playListRoot.GetLoopMin()          != pItem->m_LoopMin
		|| m_playListRoot.GetLoopMax()          != pItem->m_LoopMax
		...
		)
{
	...
	AkMusicRanSeqPlaylistItem* pItem = in_pArrayItems++;
	m_playListRoot.AvoidRepeatCount( pItem->m_wAvoidRepeatCount );
	m_playListRoot.SetLoop( pItem->m_Loop, pItem->m_LoopMin, pItem->m_LoopMax );
    ...
    for( AkUInt32 i = 0; i < in_ulNumItems; ++i )
	{
		...
		if( pItem->m_SegmentID == AK_INVALID_UNIQUE_ID ) //if not segment
		{
            ...
            if( 
				...
				|| pSub->GetLoopMin()          != pItem->m_LoopMin
				|| pSub->GetLoopMax()          != pItem->m_LoopMax
		...
		else
		{
            ...
            if( 
				...
				|| pSeg->GetLoopMin()          != pItem->m_LoopMin
				|| pSeg->GetLoopMax()          != pItem->m_LoopMax
		...
		)
{
	...
	for( AkUInt32 i = 0; i < in_ulNumItems; ++i )
	{
		...
		if( pItem->m_SegmentID == AK_INVALID_UNIQUE_ID ) //if not segment
		{
			CAkRSSub* pSub = AkNew(AkMemID_Structure, CAkRSSub( in_pParent ) );
			if( !pSub )
			{
				return AK_Fail;
			}
			...
			{
				AkDelete(AkMemID_Structure, pSub );
				return AK_Fail;
			}

			pSub->AvoidRepeatCount( pItem->m_wAvoidRepeatCount );
			pSub->SetLoop( pItem->m_Loop, pItem->m_LoopMin, pItem->m_LoopMax );
		...
		{
			//this is a segment
			CAkRSSegment* pSeg = AkNew(AkMemID_Structure, CAkRSSegment( in_pParent ) );
			if( !pSeg )
			{
				return AK_Fail;
			}
			...
			{
				AkDelete(AkMemID_Structure, pSeg );
				return AK_Fail;
			}
			pSeg->SetLoop( pItem->m_Loop, pItem->m_LoopMin, pItem->m_LoopMax );
...
CAkContainerBaseInfo* CAkMusicRanSeqCntr::GetGlobalRSInfo( AkUniqueID in_playlistId )
{
	GlobalRSInfoItem* ptr = m_globalRSInfo.Exists( in_playlistId );
	if ( NULL != ptr )
		return ptr->pRSInfo;
	return NULL;
}

bool CAkMusicRanSeqCntr::CloneGlobalRSInfo( AkUniqueID in_playlistId, const CAkContainerBaseInfo* in_pRSInfo, AkUInt16 in_numItems )
{
	AKASSERT( NULL != in_pRSInfo );
	CAkContainerBaseInfo* pClone = in_pRSInfo->Clone( in_numItems );
	if ( NULL == pClone )
		return false;

	GlobalRSInfoItem* ptr = m_globalRSInfo.Set( in_playlistId );
	if ( NULL != ptr )
	{
		if ( ptr->pRSInfo )
			ptr->pRSInfo->Destroy();
