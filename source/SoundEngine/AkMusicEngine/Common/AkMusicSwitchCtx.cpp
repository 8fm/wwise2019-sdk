...
{
    if ( CAkChainCtx::Init( in_pGameObj, in_rUserparams, in_bPlayDirectly ) == AK_Success )
    {
...
// ----------------------------------------------------------------------------------------------
AkUniqueID CAkNothingCtx::GetPlayTargetID( AkUniqueID in_playTargetID ) const
{
	if( Parent() )
...
{
	return AkNew(AkMemID_Object, CAkMusicSwitchTransition( in_pDestination ) );
}
...
void CAkMusicSwitchMonitor::SetSwitch( AkUInt32 in_switchID, const AkRTPCKey& in_rtpcKey, AkRTPCExceptionChecker* in_pExCheck )
{
	// Compute switch change only if notification applies to us.
	CAkRegisteredObj* pGameObj = m_pMusicCtx->Sequencer()->GameObjectPtr();
	if (	in_rtpcKey.GameObj() == pGameObj || 
			( in_rtpcKey.GameObj() == NULL && (!in_pExCheck || !in_pExCheck->IsException(pGameObj) ) ) ||
...
AKRESULT CAkMusicSwitchMonitor::Init( AkUInt32 in_uIdx, CAkMusicSwitchCtx& in_pMusicCtx )
{
	...
	if (res == AK_Success)
	{
		AkRTPCKey rtpcKey( in_pMusicCtx.Sequencer()->GameObjectPtr(), in_pMusicCtx.Sequencer()->PlayingID() );
		m_targetSwitchID = GetSwitchToUse( rtpcKey, uGroupID, uGroupType );
	}
...
{
    AKRESULT eResult = CAkMatrixAwareCtx::Init( in_GameObject, in_rUserparams, in_bPlayDirectly );
    ...
    )
{
	...
	for (AkTransitionsQueue::IteratorEx it = m_queueTransitions.BeginEx(); it != m_queueTransitions.End(); )
	{
	...
	if ( eResult == AK_Success )
	{
		...
		for (AkTransitionsQueue::IteratorEx it = transitionsToCancel.BeginEx(); it != transitionsToCancel.End(); )
		{
			CAkMusicSwitchTransition * pCancelledTransition = (*it);
...
CAkMatrixAwareCtx::CtxPrepareResult CAkMusicSwitchCtx::Prepare(
	...
	)
{
	...
	return m_queueTransitions.First()->Destination()->Prepare( in_pRule, in_pPlaybackHistory, io_seekingInfo, out_iRequiredLookAheadTime, out_uSelectedCue, in_uCueHashForMatchSrc, in_bWrapInSegment );
}
	...
	)
{
	...
	else if ( IsPlaying() )
	{
		...
		TransParams transParams( AkTimeConv::SamplesToMilliseconds( AkTimeConv::ToShortRange( -in_iCurrentTime ) ) );
		_Stop( transParams );
	}
	Release();
}
	...
	)
{
	...
	if ( Sequencer()->GetMusicSyncFlags() & AK_MusicSyncPoint )
	{
		...
		if ( curWindow.IsValid() )
		{
			...
			CAkScheduledItem * pItem = curWindow.GetScheduledItem();
			AKASSERT(pItem);	// Checked IsValid() above.
			pItem->GetInfo(curWindow.ToSegmentPosition(GlobalToLocalTime(Sequencer()->Now())), segmentInfo, Sequencer()->GetPlaybackSpeed());
			g_pPlayingMgr->NotifyMusic(Sequencer()->PlayingID(), AK_MusicSyncPoint, segmentInfo);
		}
	}

	MONITOR_MUSICSWITCHTRANS( 
		Sequencer()->PlayingID(), 
    ...
    )
{
	...
	if ( in_ID == AK_MUSIC_TRANSITION_RULE_ID_NONE ||
		 !pNewContext )
    {
        CAkNothingCtx * pNothingCtx = AkNew(AkMemID_Object, CAkNothingCtx( this ) );
        if ( pNothingCtx )
        {
			pNothingCtx->AddRef();
    ...
    )
{
	...
	{    
		...
		do
		{
			...
			// Update rule on stack when it is picked the first time.
			if ( rule.index != uCurRuleIndex )
			{
				uCurRuleIndex = rule.index;

				// New rule. Clean up previously allocated transitions.
				pTransCtx = NULL;
				if (pTransition1)
				{
					if (!pTransition2)
				...
				// Seek in destination info:
				// Location in segment: external seek, "last left off" (history) if applicable, or 0.
				// Jump to: rule explicit ID or last played (history), or none.
				playbackHistory = pDestParentNode
					? GetPlaybackHistory( pDestParentNode->ID() )
					: NULL;

				/// REVIEW: seek in segment and jump to segment should be used independently.
				if ( in_transitionInfo.pSeekingInfo )
					destSeekInfo.seekInSegment = *in_transitionInfo.pSeekingInfo;
				else if ( playbackHistory && ruleTo.eEntryType == EntryTypeLastExitTime )
					destSeekInfo.seekInSegment.SetAbsolute( playbackHistory->m_uTimeLeftOff );
				else
					destSeekInfo.seekInSegment.SetAbsolute( 0 );

				if ( ruleTo.eJumpToType == AkJumpToSelType_SpecificItem )
					destSeekInfo.uJumpTo = rule.destRule.uJumpToID;

				if ( bCanUseHistory && playbackHistory )
				{
					if ( ruleTo.eJumpToType == AkJumpToSelType_LastPlayedSegment ||
						ruleTo.eJumpToType == AkJumpToSelType_NextSegment )
					{
						destSeekInfo.bUseHistory = true;
					}
				}
			}
			...
			AkInt32 iDestinationLookAhead;
			CtxPrepareResult ePrepareResult = io_pNewContext->Prepare( &ruleTo, playbackHistory, destSeekInfo, iDestinationLookAhead, uDestCueHash );
			if ( ePrepareResult == CtxPrepare_Failure )
			{
				// Fatal error. 
				io_pNewContext = NULL;
				break;
			}
			else if ( ePrepareResult == CtxPrepare_SegmentChanged )
			{
				// Segment changed. Re-evaluate rule.
				eReturnVal = AK_Fail;
				continue;
			}
			...
			if ( pTransition2 )
			{
				...
				AKASSERT( pTransCtx );
				AkMusicSeekInfo seekInfoTrans;
				ePrepareResult = pTransCtx->Prepare( &ruleToTrans, NULL, seekInfoTrans, iTransitionCtxLookAhead, uTransCtxDestCueHash );
				AKASSERT( ePrepareResult == CtxPrepare_Success );	/// TODO Handle this when supporting other objects than transition segments.
...
find_sync_pos:
				...
				&& eReturnVal == AK_Success )
			{
				...
				eReturnVal = wndTrans.FindSyncPoint( 
					...
					true,	// Can find a sync point on pre-entry with transition segments.
					true,	// force succeed if source is <nothing>.
					iSyncPoint2 );
			}
				...
				&& wndSource.GetScheduledItem()->SegmentCtx() )
			{
				...
				{
					destSeekInfo.seekInSegment = wndSource.ToSegmentPosition( iSyncPoint1 );
					ePrepareResult = io_pNewContext->Prepare( 
						&ruleTo, 
						playbackHistory,
						destSeekInfo,
				...
				// TEMP: Prepare() has to succeed because we asked for wrapping around first segment.
				// Need to rewrite this algorithm in order to skip to next segment for those rules.
				AKASSERT( ePrepareResult == CtxPrepare_Success );
			...
			if ( eReturnVal == AK_Success )
			{
				...
				AkUniqueID idSrcSegment = AK_INVALID_UNIQUE_ID;	// INVALID if <nothing>, segment otherwise.
				AkUniqueID idSrcNode = AK_INVALID_UNIQUE_ID;	// Playlist or switch if <nothing>, segment otherwise.
				CAkMusicNode * pParentNode = NULL;
				CAkMusicSegment * pSegmentNode = wndSource.GetNode( &pParentNode );
				AKASSERT( pSegmentNode || pParentNode );	// Either the segment node is set, or the parent node exists.
				...
				// Store source context history for future use.
				if ( pParentNode )
				{
					AkTimeMs timeLeftOff = wndSource.ToSegmentPosition( iSyncPoint1 );
					AkUniqueID playlistId = AK_INVALID_UNIQUE_ID;

					if ( pParentNode->NodeCategory() == AkNodeCategory_MusicRanSeqCntr )
					{
						CAkScheduledItem * pSrcItem = wndSource.GetScheduledItem();
						CAkSegmentCtx * pSegmentCtx = ( pSrcItem ) ? pSrcItem->SegmentCtx() : NULL;
						playlistId = ( pSegmentCtx ) ? pSegmentCtx->GetPlayListItemID() : AK_INVALID_UNIQUE_ID;
					}

					SetPlaybackHistory( pParentNode->ID(), timeLeftOff, playlistId );
				}					
			...
			else 
			{
				...
				// Cancel (destroy) transitions and reset rules.
				uCurRuleIndex = (AkUInt32)-1;
	...
	if ( !io_pNewContext )
	{
		MONITOR_MUSICSWITCHTRANS( Sequencer()->PlayingID(), Sequencer()->GameObjectPtr()->ID(), AkMonitorData::NotificationReason_MusicTransitionNoTransNeeded, AK_UINT_MAX, m_pSwitchCntrNode->ID(), AK_INVALID_UNIQUE_ID, in_destinationID, AK_INVALID_UNIQUE_ID, AK_INVALID_UNIQUE_ID, AK_INVALID_UNIQUE_ID, AK_INVALID_UNIQUE_ID, 0 );
	}
#endif
}
...
const AkMusicTransitionRule & CAkMusicSwitchCtx::GetTransitionRule( 
	...
	CAkMatrixAwareCtx *	in_pDestContext,			// Destination context. Can be destroyed within this function (and set to NULL).
	AkUInt32 &			io_uSrcSegmentLookAhead, 	// Src look-ahead limit. Passed this limit, use panic rule.
	CAkMusicNode *&		out_pDestParentNode,		// Destination parent node for history look-up. Can be 'this' or even NULL.
	bool &				out_bCanUseHistory			// Whether playback history can be used to resolve the segment to play
	)
{
	out_pDestParentNode = NULL;
	...
	CAkScheduleWindow destination( in_pDestContext, true );
	CAkMusicNode * pDestNode = destination.GetNode( &out_pDestParentNode );
	...
	AkNodeCategory parentCategory = out_pDestParentNode ? out_pDestParentNode->NodeCategory() : AkNodeCategory_None;
	bool parentIsSeqCntr = parentCategory == AkNodeCategory_MusicRanSeqCntr;
	bool ruleDestinationIsSegment = rule.destIDs[0] == destID;
	out_bCanUseHistory = parentIsSeqCntr && !ruleDestinationIsSegment;
    return rule;
}
    ...
    )
{
	...
	AkInt32 iLookAheadTime;		// ignored.
	AkUniqueID uSelectedCue;	// ignored.
	AkMusicSeekInfo seekInfo;
	if ( Prepare( NULL, NULL, seekInfo, iLookAheadTime, uSelectedCue ) == CAkMatrixAwareCtx::CtxPrepare_Failure )
		return AK_Fail;
	return AK_Success;
}
	...
	)
{
	...
	if (it != m_queueTransitions.End()) // Must be found in the list. Pad if it isn't. WG-30855
	{
...
// Function to retrieve the play target (the node which was played, resulting in a music PBI)
// ----------------------------------------------------------------------------------------------
AkUniqueID CAkMusicSwitchCtx::GetPlayTargetID( AkUniqueID in_playTargetID ) const
{
	if( m_pSwitchCntrNode )
		in_playTargetID = m_pSwitchCntrNode->ID();
	if( Parent() )
		in_playTargetID = Parent()->GetPlayTargetID( in_playTargetID );
	return in_playTargetID;
}
...
// ----------------------------------------------------------------------------------------------
// Functions used for playback history
bool CAkMusicSwitchCtx::SetPlaybackHistory( AkUniqueID in_musicNodeId, AkTimeMs in_timeLeftOff, AkUniqueID in_playlistId )
{
	// The top level switch container ctx should keep track of playback histories.
	CAkMatrixAwareCtx* pParent = static_cast<CAkMatrixAwareCtx*>(Parent());
	if ( pParent && pParent->CanKeepPlaybackHistory() )
		return pParent->SetPlaybackHistory( in_musicNodeId, in_timeLeftOff, in_playlistId );

	// We have to store the playback history.
	CAkMusicPlaybackHistory* pHistory = m_mapNodeToHistory.Set( in_musicNodeId );
	if ( NULL != pHistory )
	{
		pHistory->m_uTimeLeftOff = in_timeLeftOff;
		pHistory->m_playlistId = in_playlistId;
		return true;
	}
...
bool CAkMusicSwitchCtx::SetPlaybackHistory( AkUniqueID in_musicNodeId, const CAkMusicPackedHistory& in_history )
{
	// The top level switch container ctx should keep track of playback histories..
	CAkMatrixAwareCtx* pParent = static_cast<CAkMatrixAwareCtx*>(Parent());
	if ( pParent && pParent->CanKeepPlaybackHistory() )
		return pParent->SetPlaybackHistory( in_musicNodeId, in_history );

	// We have to store the playback history.
	CAkMusicPlaybackHistory* pHistory = m_mapNodeToHistory.Set( in_musicNodeId );
	if ( NULL != pHistory )
	{
		pHistory->m_playlistHistory = in_history;
		return true;
	}
...
const CAkMusicPlaybackHistory* CAkMusicSwitchCtx::GetPlaybackHistory( AkUniqueID in_musicNodeId )
{
	// The top level switch container ctx should keep track of playback histories.
	CAkMatrixAwareCtx* pParent = static_cast<CAkMatrixAwareCtx*>(Parent());
	if ( pParent && pParent->CanKeepPlaybackHistory() )
		return pParent->GetPlaybackHistory( in_musicNodeId );

	// Check if we have the history.
	return m_mapNodeToHistory.Exists( in_musicNodeId );
}
