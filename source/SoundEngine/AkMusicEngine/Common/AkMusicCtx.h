...
public:
    ...
    void Release();
	AkUInt32 RefCount() const { return m_uRefCount; }
...
public:
	...
	// Notify generic property change to child contexts.
	...
	// Returns false if context is playing or idle, true if in its last frame or already stopped.
	inline bool IsStopped() { return ( m_eStatus == CtxStatusStopped ); }

	// Returns true if context is (completely) paused.
	inline bool	IsPaused() { return m_bIsPaused; }

	// Returns true if ctx has a play-stop transition on it.
	bool HasStopTrans();
