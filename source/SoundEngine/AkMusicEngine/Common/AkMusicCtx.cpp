    ...
    ,m_bIsPaused( false )	
{
	...
	AKASSERT( !in_parent || !in_parent->IsStopped() );
}
...
void CAkMusicCtx::Release()
{
    ...
    if ( m_uRefCount == 0 )
    {
        if ( m_pParentCtx )
		{
            ...
            CAkMusicRenderer::RemoveChild( (CAkMatrixAwareCtx*)this ); /// TEMP. Templatize child/parent/music contexts. Avoid casts.
    ...
    )
{
    ...
    CAkMusicRenderer::NotifyStateChangeStartPlay( this );
    **/
	...
	)							// 0 is "immediate", AK_NO_IN_BUFFER_STOP_REQUESTED is "one audio frame with minimal transition time". 
{
	...
	else if ( m_PBTrans.pvPSTrans )
    {
		...
		{
			//Immediate stop was requested, kill the transition.
			g_pTransitionManager->RemoveTransitionUser(m_PBTrans.pvPSTrans, this);
			m_PBTrans.pvPSTrans = NULL;
			OnLastFrame(in_uNumLastSamples);
		}
    ...
    )
{
    ...
    {       
        ...
        else
		{
			...
			m_fPauseResumeFadeRatio = AK_UNMUTED_RATIO;
			AkDeltaMonitor::OpenUpdateBrace(AkDelta_Fade);
			SetPBIFade( &m_fPauseResumeFadeRatio, m_fPauseResumeFadeRatio );
...
void CAkMusicCtx::_NotifyPropertyChange(
	AkPropID in_eProp,		// Property.
	AkReal32 in_fValue		// New value.
	)
{
	OnPropertyChange( in_eProp, in_fValue );
}

// Broadcast trigger modulators request to child contexts.
void CAkMusicCtx::_TriggerModulators()
{
	OnTriggerModulators();
}
...
void CAkMusicCtx::_Stop( AkPBIStopMode, bool )
{
...
bool CAkMusicCtx::HasStopTrans()
{
	return m_PBTrans.pvPSTrans && m_PBTrans.pvPSTrans->IsFadingOut();
}
...
// Broadcast generic property change to child contexts.
void CAkMusicCtx::OnPropertyChange(
	AkPropID in_eProp,		// Property.
	AkReal32 in_fValue		// New value.
	)
{
    ...
    while ( it != m_listChildren.End( ) )
    {
		CAkChildCtx* pChildCtx = *it;
		++it;
        pChildCtx->OnPropertyChange( in_eProp, in_fValue );
    }
}

// Broadcast trigger modulators request to all child contexts.
void CAkMusicCtx::OnTriggerModulators()
{
	ChildrenCtxList::Iterator it = m_listChildren.Begin( );
	while ( it != m_listChildren.End( ) )
	{
