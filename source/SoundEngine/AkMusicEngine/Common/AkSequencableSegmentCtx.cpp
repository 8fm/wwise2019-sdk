    ...
    )
{
	...
	eResult = SetupSegmentChain( in_GameObject, in_rUserparams, iStartPos, iLookAheadDuration, in_bPlayDirectly );
    ...
    )
{
    ...
    CAkSegmentCtx * pChildCtx = m_pSegmentNode->CreateLowLevelSegmentCtxAndAddRef( this, in_GameObject, in_rUserparams, in_bPlayDirectly );
	...
	AkMusicSeekInfo seekInfo = AkMusicSeekInfo( in_iStartPos );
	CtxPrepareResult ePrepareResult = Prepare( NULL, NULL, seekInfo, out_iLookAheadDuration, uCueDest );
	if ( AK_EXPECT_FALSE( ePrepareResult == CtxPrepare_Failure ) )
		return AK_Fail;
	...
	)
{
	...
	if ( pItem )
		return pItem->GetInfo(window.ToSegmentPosition(GlobalToLocalTime(Sequencer()->Now())), out_segmentInfo, Sequencer()->GetPlaybackSpeed());
	return AK_Fail;
}
...
// ----------------------------------------------------------------------------------------------
AkUniqueID CAkSequencableSegmentCtx::GetPlayTargetID( AkUniqueID in_playTargetID ) const
{
	if( m_pSegmentNode )
