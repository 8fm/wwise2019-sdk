...
#include "../../../AkAudiolib/Common/IAkGlobalPluginContextEx.h"

using namespace AK;

// Plugin mechanism. Implement Create function and register its address to the FX manager.
IAkPluginParam * CreateAkSilenceSourceParams( IAkPluginMemAlloc * in_pAllocator )
{
    return AK_PLUGIN_NEW( in_pAllocator, CAkFxSrcSilenceParams( ) );
}

#define AKSOURCEID_SILENCE 101

AK_IMPLEMENT_PLUGIN_FACTORY(AkSilenceSource, AkPluginTypeSource, 0, AKSOURCEID_SILENCE)
...
//-----------------------------------------------------------------------------
IAkPlugin* CreateAkSilenceSource( IAkPluginMemAlloc * in_pAllocator )
{
...
//-----------------------------------------------------------------------------
void CAkFXSrcSilence::Execute(	AkAudioBuffer * io_pBufferOut )
{
...
AkReal32 CAkFXSrcSilence::RandRange( AkReal32 in_fMin, AkReal32 in_fMax )
{
	...
	AkReal32 fRandVal = static_cast<AK::IAkGlobalPluginContextEx *>(m_pSourceFXContext->GlobalContext())->Random() / static_cast<AkReal32>(AKRANDOM::AK_RANDOM_MAX);
