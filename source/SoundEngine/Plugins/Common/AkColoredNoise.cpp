...
namespace DSP
{
	void CAkColoredNoise::Init(AkNoiseColor in_eNoiseColor, AkUInt32 in_uSampleRate, AkUInt32 in_uLCGState)
	{
		CAkWhiteNoise::Init(in_uLCGState);
		...
		AkUInt32 in_uNumFrames )
	{
		...
		{	
			...
			uIndex = (uIndex + 1) & AKPINKNOISE_INDEXMASK;
	...
	{
		CAkWhiteNoise::GenerateBuffer( out_pfBuffer, in_uNumFrames );
		PurpleFilter.ProcessBufferMono(out_pfBuffer, in_uNumFrames);
	}
