...
namespace DSP
{
	bool CAkOLACircularBuffer::IsDoneTail( )
	{
		...
		if ( uTailEndPos >= m_uReadOffset )
			...
			uFramesToFlush = m_uSize - (m_uReadOffset - uTailEndPos);
		...
		AkReal32 * io_pfData )
	{
		...
		{
			AkReal32 * AK_RESTRICT pfOLAData = (AkReal32 * ) &io_pfData[m_uWriteOffset];
			AkReal32 * AK_RESTRICT pfInData = (AkReal32 * ) in_pfBuffer;
		...
		{
			AkReal32 * AK_RESTRICT pfOLAData = (AkReal32 * ) &io_pfData[0];
			AkReal32 * AK_RESTRICT pfInData = (AkReal32 * ) in_pfBuffer+uFramesBeforeWrap;
