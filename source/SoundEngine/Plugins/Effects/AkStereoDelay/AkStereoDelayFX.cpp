...
AK::IAkPlugin* CreateAkStereoDelayFX( AK::IAkPluginMemAlloc * in_pAllocator )
{
...
{
	// This effect does not support channel configs that do not have at least a named front left or front center channel.
	if ((in_rFormat.channelConfig.uChannelMask & AK_SPEAKER_FRONT_LEFT) == 0 && (in_rFormat.channelConfig.uChannelMask & AK_SPEAKER_FRONT_CENTER) == 0)
...
{												\
	...
	m_FXInfo.uMaxBufferSize = in_pFXCtx->GlobalContext()->GetMaxBufferLength();
