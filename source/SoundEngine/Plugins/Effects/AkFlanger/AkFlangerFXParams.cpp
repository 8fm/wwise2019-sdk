...
// Creation function
AK::IAkPluginParam * CreateAkFlangerFXParams( AK::IAkPluginMemAlloc * in_pAllocator )
{
                                               ...
                                               )
{
	...
	// Normalize wet/dry mix
	m_Params.RTPC.fWetDryMix *= 0.01f;
                                         ...
                                         )
{
	...
	switch ( in_ParamID )
	{
		...
		case AK_FLANGERFXPARAM_DRYLEVEL_ID:
		{
			...
			m_Params.RTPC.fDryLevel = AkClamp( fValue, 0.f, 1.f );
		...
		{
			AkReal32 fValue = *((AkReal32*)in_pValue);
			m_Params.RTPC.fFfwdLevel = AkClamp( fValue, -1.f, 1.f );
			m_Params.RTPC.SetDirty( true );
			break;
		}
		...
		{
			AkReal32 fValue = *((AkReal32*)in_pValue);
			m_Params.RTPC.fFbackLevel = AkClamp( fValue, -1.f, 1.f );
			m_Params.RTPC.SetDirty( true );
			break;
		}
		case AK_FLANGERFXPARAM_MODDEPTH_ID:
		{
			...
			m_Params.RTPC.fModDepth = fValue * 0.01f;
			m_Params.RTPC.SetDirty( true );
			break;
		}
		...
		case AK_FLANGERFXPARAM_MODSMOOTHING_ID:
		{
			AkReal32 fValue = *((AkReal32*)in_pValue);
			fValue = AkClamp( fValue, 0.f, 100.f );
			m_Params.RTPC.modParams.lfoParams.fSmooth = fValue * 0.01f;
			m_Params.RTPC.SetDirty( true );
			break;
		}
		case AK_FLANGERFXPARAM_MODPWM_ID:
		{
			AkReal32 fValue = *((AkReal32*)in_pValue);
			fValue = AkClamp( fValue, 0.f, 100.f );
			m_Params.RTPC.modParams.lfoParams.fPWM = fValue * 0.01f;
			m_Params.RTPC.SetDirty( true );
			break;
		}
		...
		{
			AkReal32 fValue = *((AkReal32*)in_pValue);
			m_Params.RTPC.modParams.phaseParams.fPhaseOffset = AkClamp( fValue, -180.f, 180.f );
			break;
		}
		...
		{
			AkReal32 fValue = *((AkReal32*)in_pValue);
			m_Params.RTPC.modParams.phaseParams.fPhaseSpread = AkClamp( fValue, 0.f, 180.f );
			break;
		}
		case AK_FLANGERFXPARAM_OUTPUTLEVEL_ID:
		{
			...
			m_Params.RTPC.fOutputLevel = AK_DBTOLIN( fValue );
			m_Params.RTPC.SetDirty( true );
			break;	
		}
		case AK_FLANGERFXPARAM_WETDRYMIX_ID:
		{
			AkReal32 fValue = *((AkReal32*)in_pValue);
			fValue = AkClamp( fValue, 0.f, 100.f );
			m_Params.RTPC.fWetDryMix = fValue * 0.01f;
			m_Params.RTPC.SetDirty( true );
			break;
		}
		// Non-RTPC params
