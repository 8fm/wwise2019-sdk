...
// Creation function
AK::IAkPluginParam * CreateAkParametricEQFXParams( AK::IAkPluginMemAlloc * in_pAllocator )
{
											...
											AkUInt32 in_ulParamSize )
{
	...
	case AK_PARAMETRICEQFXPARAM_GAINBAND3_ID:
	{
		...
		m_Params.Band[ulBandNum].fGain = AkClamp( fValue, -24.f, 24.f );
		AKASSERT( ulBandNum <= BAND3 );
		m_bBandDirty[ulBandNum] = true;
		break;
	}
	...
	{
		AkReal32 fValue = *reinterpret_cast<const AkReal32*>(in_pValue);
		m_Params.fOutputLevel = AkClamp( fValue, -24.f, 24.f );
		break;
	}
