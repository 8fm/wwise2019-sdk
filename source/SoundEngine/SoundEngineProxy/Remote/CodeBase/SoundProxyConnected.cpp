...
void SoundProxyConnected::HandleExecute( AkUInt16 in_uMethodID, CommandDataSerializer& in_rSerializer, CommandDataSerializer& out_rReturnSerializer )
{
	...
	case ISoundProxy::MethodSetNonCachable:
		{
			SoundProxyCommandData::SetNonCachable setNonCachable;
			if( in_rSerializer.Get( setNonCachable ) )
				pSound->SetNonCachable( setNonCachable.m_param1 );
			break;
		}
