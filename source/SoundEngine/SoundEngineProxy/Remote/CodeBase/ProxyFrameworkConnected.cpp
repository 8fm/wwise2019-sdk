...
AK::Comm::IProxyFrameworkConnected* AK::Comm::IProxyFrameworkConnected::Create()
{
	return AkNew(AkMemID_Profiler, ProxyFrameworkConnected());
}
...
const AkUInt8* ProxyFrameworkConnected::HandleExecute( const AkUInt8* in_pData, AkUInt32 & out_uReturnDataSize )
{
	...
	else if( proxyCmdData.m_commandType == ProxyCommandData::TypeMidiDeviceMgrProxy )
	{
		m_midiDeviceMgrProxy.HandleExecute( serializer, m_returnData );
	}
...
#define PROXY_CAST_ALLOCATION_CONSTRUCTOR( _Creation_Type_, _What_ ) \
	pProxyItem = (ID2ProxyConnected::Item *)  AkAlloc( AkMemID_Profiler, lProxyItemOffset + sizeof( _Creation_Type_ ) ); \
	if( pProxyItem )\
	{\
...
void ProxyFrameworkConnected::ProcessProxyStoreCommand( CommandDataSerializer& io_rSerializer )
{
	...
	if( cmdData.m_methodID == ObjectProxyStoreCommandData::MethodCreate )
	{
		...
		case ObjectProxyStoreCommandData::TypeModulatorLfo:
			PROXY_CAST_ALLOCATION( ModulatorLfoProxyConnected );
			break;

		case ObjectProxyStoreCommandData::TypeModulatorEnvelope:
			PROXY_CAST_ALLOCATION( ModulatorEnvelopeProxyConnected );
			break;

		case ObjectProxyStoreCommandData::TypeModulatorTime:
			PROXY_CAST_ALLOCATION(ModulatorTimeProxyConnected);
			break;

		case ObjectProxyStoreCommandData::TypeFxShareSet:
			PROXY_CAST_ALLOCATION1(FxBaseProxyConnected, FxBaseProxyType_EffectShareset);
			break;

		case ObjectProxyStoreCommandData::TypeFxCustom:
			PROXY_CAST_ALLOCATION1(FxBaseProxyConnected, FxBaseProxyType_EffectCustom);
			break;

		case ObjectProxyStoreCommandData::TypeAudioDevice:
			PROXY_CAST_ALLOCATION1(FxBaseProxyConnected, FxBaseProxyType_AudioDevice);
			break;

		case ObjectProxyStoreCommandData::TypeAcousticTexture:
			PROXY_CAST_ALLOCATION(AcousticTextureProxyConnected);
			break;

		default:
			if( g_pExternalProxyHandler != NULL )
			{
				g_pExternalProxyHandler( create, pProxyItem, lProxyItemOffset, AkMemID_Profiler);
			}
			break;
		}
...
ProxyFrameworkConnected::ID2ProxyConnected::Item* ProxyFrameworkConnected::CreateAction( AkUniqueID in_actionID, AkActionType in_eActionType )
{
	...
	case ACTION_TYPE_PLAYEVENT:
	case ACTION_TYPE_RESET_PLAYLIST:
	...
	case ACTION_TYPE_SETHPF:
	case ACTION_TYPE_RESETHPF:
