...
ActionProxyConnected::ActionProxyConnected( AkActionType in_eActionType, AkUniqueID in_uID )
{
	...
	if(pIndexable)
	{
		CAkAction* pAction = (CAkAction*)pIndexable;
		if (pAction->ActionType() != in_eActionType)
		{
			pAction->RemoveFromIndex();// Removing the old Action from the index, turning it into some temporary state action.
			pAction->Release();
			pIndexable = NULL;
			pAction = NULL;
		}
...
void ActionStopProxyConnected::HandleExecute( AkUInt16 in_uMethodID, CommandDataSerializer& in_rSerializer, CommandDataSerializer& out_rReturnSerializer )
{
	CAkActionStop * pActionStop = static_cast<CAkActionStop *>( GetIndexable() );
	
	switch( in_uMethodID )
	{
	case IActionStopProxy::MethodApplyToStateTransitions:
		{
			ActionStopProxyCommandData::ApplyToStateTransitions applyToStateTransitions;
			if( in_rSerializer.Get( applyToStateTransitions ) )
				pActionStop->ApplyToStateTransitions( applyToStateTransitions.m_param1 );
			break;
		}
	case IActionStopProxy::MethodApplyToDynamicSequences:
		{
			ActionStopProxyCommandData::ApplyToDynamicSequences applyToDynamicSequences;
			if( in_rSerializer.Get( applyToDynamicSequences ) )
				pActionStop->ApplyToDynamicSequence( applyToDynamicSequences.m_param1 );
			break;
		}
...
void ActionPauseProxyConnected::HandleExecute( AkUInt16 in_uMethodID, CommandDataSerializer& in_rSerializer, CommandDataSerializer& out_rReturnSerializer )
{
	...
	case IActionPauseProxy::MethodApplyToStateTransitions:
		{
			ActionPauseProxyCommandData::ApplyToStateTransitions applyToStateTransitions;
			if( in_rSerializer.Get( applyToStateTransitions ) )
				pActionPause->ApplyToStateTransitions( applyToStateTransitions.m_param1 );
			break;
		}
	case IActionPauseProxy::MethodApplyToDynamicSequences:
		{
			ActionPauseProxyCommandData::ApplyToDynamicSequences applyToDynamicSequences;
			if( in_rSerializer.Get( applyToDynamicSequences ) )
				pActionPause->ApplyToDynamicSequence( applyToDynamicSequences.m_param1 );
			break;
		}
...
void ActionResumeProxyConnected::HandleExecute( AkUInt16 in_uMethodID, CommandDataSerializer& in_rSerializer, CommandDataSerializer& out_rReturnSerializer )
{
	...
	case IActionResumeProxy::MethodApplyToStateTransitions:
		{
			ActionResumeProxyCommandData::ApplyToStateTransitions applyToStateTransitions;
			if( in_rSerializer.Get( applyToStateTransitions ) )
				pActionResume->ApplyToStateTransitions( applyToStateTransitions.m_param1 );
			break;
		}
	case IActionResumeProxy::MethodApplyToDynamicSequences:
		{
			ActionResumeProxyCommandData::ApplyToDynamicSequences applyToDynamicSequences;
			if( in_rSerializer.Get( applyToDynamicSequences ) )
				pActionResume->ApplyToDynamicSequence( applyToDynamicSequences.m_param1 );
			break;
		}
...
void ActionSetGameParameterProxyConnected::HandleExecute( AkUInt16 in_uMethodID, CommandDataSerializer& in_rSerializer, CommandDataSerializer& out_rReturnSerializer )
{
	...
	case IActionSetGameParameterProxy::MethodBypassGameParameterInternalTransition:
		{
			ActionSetGameParameterProxyCommandData::BypassGameParameterInternalTransition bypassGameParameterInternalTransition;
			if( in_rSerializer.Get( bypassGameParameterInternalTransition ) )
				pActionSetGameParameter->BypassGameParameterInternalTransition( bypassGameParameterInternalTransition.m_param1 );
			break;
		}
