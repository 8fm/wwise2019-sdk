...
// WG-30217 Fixed for Mac only because it is a dev platform.
// The problem is not likely to happen on other platforms.
#ifdef AK_MAC_OS_X
	::setsockopt( m_socket, SOL_SOCKET, SO_REUSEPORT, (char*)&bReuseAddr, sizeof bReuseAddr );
#else
...
{
	AkUInt32 iMode = 1;
	return ioctlsocket(m_socket, FIONBIO, &iMode) == NO_ERROR;	
}
...
{
	return ::recv(m_socket, (char*)in_pBuf, in_length, in_flags);
}
