...
{
	int flag = 1;
	return sceNetSetsockopt(m_socket, IPPROTO_TCP, SCE_NET_SO_NBIO, (char *)&flag, sizeof(int)) == 0;
}
...
{
	int res = sceNetRecv( m_socket, in_pBuf, in_length, in_flags);
...
{
	return Recv(in_pBuf, in_length, SCE_NET_MSG_WAITALL);
}
...
AkInt32 GameSocket::Poll( PollType in_ePollType, AkUInt32 in_timeout ) const
{
	if (m_epoll == 0)
	{
		m_epoll = sceNetEpollCreate("akepoll", 0);
		if (m_epoll < 0)
		{
			m_epoll = 0;
			return SOCKET_ERROR;
		...
		memset(&m_pollEvent, 0, sizeof(m_pollEvent));
		m_pollEvent.events = (in_ePollType == PollRead) ? SCE_NET_EPOLLIN : SCE_NET_EPOLLOUT;
		// balary TODO pollEvent.data.ext.id = m_socket;

		int res = sceNetEpollControl(m_epoll, SCE_NET_EPOLL_CTL_ADD, m_socket, &m_pollEvent);
		if (res < 0)
		{
	...
	int res = sceNetEpollWait(m_epoll, &m_pollEvent, 1, in_timeout );

	if ( res > 0 )
	{
		if (0 == (m_pollEvent.events & ( (in_ePollType == PollRead) ? SCE_NET_EPOLLIN : SCE_NET_EPOLLOUT ) ))
		{
			return SOCKET_ERROR;
		}
