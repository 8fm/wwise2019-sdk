	...
	if(info->type<2)
	{
		...
		if(n>0)
		{
			...
			if(used)
			{
				...
				char * partword[ AK_VOICE_MAX_NUM_CHANNELS ];
#else
				char ** partword = (char **) alloca(ch * sizeof(char *));
#endif
						...
						for(k=0;k<partitions_per_word && i<partvals;k++,i++)
						{
							for(j=0;j<ch;j++)
							{
								...
								if(info->stagemasks[(int)partword[j][i]]&(1<<s))
								{
									...
									AKSIMD_PREFETCHMEMORY(0, stagebook->dec_table);
									ogg_int32_t * data = in[j] + offset;									
									stagebook->pResidueParams->Decode(stagebook, &data, 0, GET_OGGPACK_BUFFER(), samples_per_partition);									
								}
							}
						}
					}
			...
			for(s=0;s<info->stages;s++)
			{
				for(i=0;i<partvals;)
				{
					...
					for(k=0;k<partitions_per_word && i<partvals;k++,i++)
					{
						...
						{
							codebook *stagebook=ci->book_param + info->stagebooks[(partword[i]<<3)+s];							
							stagebook->pResidueParams->Decode(stagebook, in, i*samples_per_partition + beginoff, GET_OGGPACK_BUFFER(), samples_per_partition);
						}
					}
				}
			}
