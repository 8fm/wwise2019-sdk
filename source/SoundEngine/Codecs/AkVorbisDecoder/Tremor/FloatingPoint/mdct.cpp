...
{
	float	*T = fsincos_lookup0;
	float	*x1	= x + points - 4;
	float	*x2	= x + (points>>1) - 4;
	float r0, r1, r2, r3;
...
extern void mdct_backward_SIMD(int n, float *in, int end);
//====================================================================================================
						...
						int			end    /* samples, this frame */)
{
	...
	//const float fFactor = (1/512.0f)/32768.f;
	const float fFactor = (1/256.0f);	//This factor changes because the dbToLin function in render_line() is no longer integer (0-65536) but 0-1 in float.
