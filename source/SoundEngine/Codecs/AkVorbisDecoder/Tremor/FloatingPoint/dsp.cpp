...
float *g_mdctBuffer = NULL;
char *g_pWorkBuffer = NULL;
int g_cWorkSize = 0;
const int c_OneChannelWindowSize = 2048*sizeof(float);	//Max window size supported is 2048.
	...
	)
{
	if(g_pWorkBuffer)
	{
		AkFalign(AkMemID_Processing, g_pWorkBuffer);
		g_pWorkBuffer = NULL;
		g_cWorkSize = 0;
	}
	...
	if(g_mdctBuffer)
	{
		AkFalign(AkMemID_Processing, g_mdctBuffer);
		g_mdctBuffer = NULL;
	}
}
...
int vorbis_dsp_init(vorbis_dsp_state* v, int channels )
{
	...
	v->work = (ogg_int32_t **)AkAlloc(AkMemID_Processing, sizeof(ogg_int32_t *) * channels * 2);
	if (!v->work)
		return -1;
	...
	char* pDct		= (char*)_ogg_malloc(DctSize);
	...
	//Allocate temporary buffer for mdct_backward_SIMD.  We only need one for all vorbis instances.  Max window size supported is 2048.
	if (g_mdctBuffer == NULL)
	{
		g_mdctBuffer = (float*)AkMalign(AkMemID_Processing, c_OneChannelWindowSize, AK_SIMD_ALIGNMENT);
		if (g_mdctBuffer == NULL)
			return -1;

		AK::SoundEngine::RegisterGlobalCallback( VorbisLibTerm, AkGlobalCallbackLocation_Term );	//In case of failure, we'll leak at termination.  No biggie.
	}

	//Allocate the space for the temporal data for all channels.
	//If more channels are needed re-alloc (keeping the old one temporarily so previous streams can still play if there is not enough memory).
	int WorkSize = c_OneChannelWindowSize * 2 * channels;	//Need twice the channel space because of real and imaginary parts of the spectrum.
	if (WorkSize > g_cWorkSize || g_pWorkBuffer == NULL)
	{
		char *pTmp = (char*)AkMalign(AkMemID_Processing, WorkSize, AK_SIMD_ALIGNMENT);
		if (pTmp)
		{
			if (g_pWorkBuffer)
				AkFalign(AkMemID_Processing, g_pWorkBuffer);
			...
			g_pWorkBuffer = pTmp;
			g_cWorkSize = WorkSize;			
		}
		else
			return -1;
	}

	if (g_pWorkBuffer == NULL)
	{
		g_cWorkSize = 0;
		return -1;
	}
...
void vorbis_dsp_clear(vorbis_dsp_state *v)
{
...
#endif
	{
		...
		{
			// Do not count in UVM, do not alloc in UVM memory
			_ogg_free(v->mdctright[0]);
			v->mdctright[0] = NULL;
		}

#ifndef AK_VOICE_MAX_NUM_CHANNELS
		AkFree(AkMemID_Processing, v->work);
		v->work = NULL;
		v->mdctright = NULL;
#endif
	}
