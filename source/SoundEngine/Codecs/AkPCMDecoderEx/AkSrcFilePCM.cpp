...
void CAkSrcFilePCM::GetBuffer( AkVPLState & io_state )
{
	...
	if ( 0 == m_uNumBytesBuffered )
	{
			...
			&& uSizeLeftAfterRelease > 0 )
		{
			...
			AKASSERT(m_pCtx->GetSrcTypeInfo()->mediaInfo.Type == SrcTypeFile);
	...
	SubmitBufferAndUpdate( pSubmitData, uMaxFrames, rFormatInfo.uSampleRate, rFormatInfo.channelConfig, io_state );
}
	...
	)
{
	...
	//Check if the file is of the expected format: AK_WAVE_FORMAT_EXTENSIBLE
	if (fmtInfo.pFormat->wFormatTag != AK_WAVE_FORMAT_EXTENSIBLE)
	{
	...
	// Important: fmtInfo.uFormatSize may be larger than WaveFormatExtensible because this PCM media may have been decoded on bank load from a different codec (and thus may have different format sizes).
...
{
	return m_bIsReadingPrefecth || m_bIsMemoryStream;
}
