...
static int op_get_data(OggOpusFile *_of,int _nbytes){
  ...
  if (buffer == NULL)  // Audiokinetic
	  return -1;       // Audiokinetic
...
static int op_make_decode_ready(OggOpusFile *_of){
  ...
  //RED
#if !defined(OP_FIXED_POINT) //RED && !defined(AUDIOKINETIC)
...
void op_decode_clear(OggOpusFile *_of){
...
static int op_open1(OggOpusFile *_of,
  ...
  *&_of->callbacks=*_cb;
  ...
  if (OP_UNLIKELY(_of->links == NULL))return OP_EFAULT; /*AUDIOKINETIC*/
...
static int op_pcm_seek_page(OggOpusFile *_of,
 ogg_int64_t _target_gp,int _li){
  ...
  while(begin<end){
    ...
    if(end-begin<OP_CHUNK_SIZE)bisect=begin;
      ...
      if(force_bisect)bisect=begin+(end-begin>>1);
		...
		bisect = begin + op_rescale64(diff, diff2, end - begin);
		if(bisect > _of->end - OP_CHUNK_SIZE)
			bisect = _of->end - OP_CHUNK_SIZE;	//AUDIOKINETIC: if seeking near the end, get the last page, period.
		else 
			bisect -= OP_CHUNK_SIZE;
      }
      if(bisect<begin)bisect=begin;
      force_bisect=0;
    }
  ...
  if(OP_UNLIKELY(ret == OP_EREAD))	//AUDIOKINETIC
	  return OP_EREAD;
  if(OP_UNLIKELY(ret<0))
	  return OP_EBADLINK;
  /*Verify result.*/
...
/*AUDIOKINETIC
  Optimized variant of op_read_float/op_read_native; expects client to consume full packets,
  and avoids memcpy/memmove by returning pointer to internal work buffer.
*/
int AK_op_read_float_no_copy(OggOpusFile *_of,
	op_sample **_pcm, int * _packetduration) {
	if (OP_UNLIKELY(_of->ready_state<OP_OPENED))return OP_EINVAL;
	for (;;) {
		int ret;
		if (OP_LIKELY(_of->ready_state >= OP_INITSET)) {
			...
			nchannels = _of->links[_of->seekable ? _of->cur_link : 0].head.channel_count;
			od_buffer_pos = _of->od_buffer_pos;
			OP_ASSERT((_of->od_buffer_size - od_buffer_pos) == 0); // there should be no buffered samples
			/*If we have buffered packets, decode one.*/
			op_pos = _of->op_pos;
			if (OP_LIKELY(op_pos<_of->op_count)) {
				const ogg_packet *pop;
				...
				_of->prev_packet_gp = pop->granulepos;
				{
					...
					if (OP_LIKELY((trimmed_duration - od_buffer_pos)>0)) {
						*_pcm = buf + od_buffer_pos*nchannels;
						*_packetduration = duration;
						return trimmed_duration - od_buffer_pos;
					}
				}
				/*Don't grab another page yet.
				This one might have more packets, or might have buffered data now.*/
...
/**************** AUDIOKINETIC BEGIN ********************/
OggOpusFile *AK_op_open_callbacks(void *_stream, const OpusFileCallbacks *_cb,
	const unsigned char *_initial_data, size_t _initial_bytes, int *_error){
	OggOpusFile *of;
	int          ret;
	of = (OggOpusFile *)_ogg_malloc(sizeof(*of));
	...
	if(_error != NULL)*_error = ret;
	if(ret != 0 && ret != OP_EREAD && of) /*AUDIOKINETIC*/
	{
		/*Don't auto-close the stream on failure.*/
		of->callbacks.close = NULL;
		op_clear(of);
...
int AK_continue_open(OggOpusFile *_of)
{
	ogg_page  og;
	int ret = op_find_initial_pcm_offset(_of, _of->links, &og);
	if(ret == 0)
	{
