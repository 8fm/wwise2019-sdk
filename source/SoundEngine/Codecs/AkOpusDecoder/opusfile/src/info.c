...
int opus_head_parse(OpusHead *_head,const unsigned char *_data,size_t _len){
  ...
  else if(head.mapping_family==1 || head.mapping_family == 2 || head.mapping_family == 255){
    size_t size;
    int    ci;
    if(head.channel_count<1||((head.mapping_family == 1)&&(head.channel_count>8)))return OP_EBADHEADER;
