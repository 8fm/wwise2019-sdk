...
AKRESULT CAkSinkDirectSound::Init(AkPlatformInitSettings & in_settings, AkChannelMask in_uChannelMask)
{
	...
	{
#if defined (_DEBUG)
		AKPLATFORM::OutputDebugMsg("WARNING: Unable to Create DirectSound Instance\n");
#endif
		return AK_Fail;
	}
	...
	{
#if defined (_DEBUG)
		AKPLATFORM::OutputDebugMsg("WARNING: Unable to initialize DirectSound\n");
#endif
		return AK_Fail;
	}
	...
	m_SpeakersConfig.SetStandard(in_uChannelMask);

	if( !m_SpeakersConfig.IsValid() )
	{
		m_SpeakersConfig.SetStandard(GetDSSpeakerConfig());
	}

	if( m_SpeakersConfig.uChannelMask != AK_SPEAKER_SETUP_7POINT1 && 
		m_SpeakersConfig.uChannelMask != AK_SPEAKER_SETUP_5POINT1 && 
		m_SpeakersConfig.uChannelMask != AK_SPEAKER_SETUP_4_0 && 
		 m_SpeakersConfig.uChannelMask != AK_SPEAKER_SETUP_STEREO )
	{
		// Default to stereo
	...
	AK_PC_WAIT_TIME = ((AkUInt32)(1000.0f * AK_DEFAULT_NUM_SAMPLES_PER_FRAME / DEFAULT_NATIVE_FREQUENCY / 4.0));

	return AllocBuffer(m_SpeakersConfig);
}
...
AkChannelMask CAkSinkDirectSound::GetDSSpeakerConfig()
{
	...
	if( SUCCEEDED( hr ) )
	{
		...
		case DSSPEAKER_QUAD:
			channelConfig = AK_SPEAKER_SETUP_4_0;
			break;
...
AKRESULT CAkSinkDirectSound::Term( AK::IAkPluginMemAlloc * in_pAllocator )
{
...
void CAkSinkDirectSound::PrepareFormat( AkChannelConfig in_channelConfig, WAVEFORMATEXTENSIBLE & out_wfExt )
{
	memset( &out_wfExt, 0, sizeof( WAVEFORMATEXTENSIBLE ) );

	AkUInt16 uNumChannels = in_channelConfig.uNumChannels;
	...
	if ( in_channelConfig.uChannelMask == AK_SPEAKER_SETUP_STEREO )
	{
	...
	else // Use WAVEFORMATEXTENSIBLE for surround, as Vista+ requires it.
	{
		...
		out_wfExt.dwChannelMask					= in_channelConfig.uChannelMask;
...
HRESULT CAkSinkDirectSound::CreateSecondaryBuffer()
{
	...
	AkChannelConfig channelConfig = GetSpeakerConfig();
	
	AkUInt16 uChannels = (AkUInt16)channelConfig.uNumChannels;
	...
	PrepareFormat( channelConfig, wfExt );
...
AKRESULT CAkSinkDirectSound::Reset()
{
...
AKRESULT CAkSinkDirectSound::SetPrimaryBufferFormat()
{
	...
	do
	{
		...
		if( FAILED( hr ) )
		{
			switch( m_SpeakersConfig.uChannelMask )
			{
...
//====================================================================================================
//====================================================================================================
void CAkSinkDirectSound::PassData()
{
	if ( !m_pDSBuffer ) 
		return;
	...
	if(m_MasterOut.uValidFrames != 0)
	{
		...
		Params.ulBytes = AK_NUM_VOICE_REFILL_FRAMES * GetSpeakerConfig().uNumChannels * sizeof( AkInt16 );
...
//====================================================================================================
//====================================================================================================
void CAkSinkDirectSound::PassSilence()
{
	if ( !m_pDSBuffer ) 
		return;
...
{	
	...
	AkUInt32 uNumChannelDS = GetSpeakerConfig().uNumChannels;
	...
	if ( in_AudioBuffer.GetChannelConfig() == GetSpeakerConfig() )
	{
		FloatsToShorts((AkInt16 *) pvRefillPosition, (float *) pvFrom, ulToRefillSamples, 1, 1 );
	}
	else // Mismatch in channel config: happens when hardware device has changed
	{
		AkUInt32 uNumChannelSrc = in_AudioBuffer.NumChannels();
		...
		{
			AKASSERT( GetSpeakerConfig().uChannelMask == AK_SPEAKER_SETUP_STEREO );
...
namespace AK
{
	...
	{		
		return CAkSinkDirectSound::GetDirectSoundInstance();
	}
};
