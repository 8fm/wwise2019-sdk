...
// HW codecs can not have less then 1 Wwise update of lookahead time because it has a 1 update latency.
// The music can be sped up 4X, so this means we may have an effective frame time of up to 4 audio frames.
#define AK_IM_HW_CODEC_MIN_LOOKAHEAD_ATRAC9     (g_PDSettings.bHwCodecLowLatencyMode ? 0 : 4)
#define AK_IM_HW_CODEC_MIN_LOOKAHEAD_OPUS_WEM   (g_PDSettings.bHwCodecLowLatencyMode ? 0 : 4)
