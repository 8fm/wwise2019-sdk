...
AKRESULT AkDecisionTree::SetTree( void* in_pData, AkUInt32 in_uSize, AkUInt32 in_uDepth )
{
	...
	if( in_uSize )
	{
		m_pNodes = (AkDecisionTree::Node *) AkAlloc(AkMemID_Event, in_uSize);
		if ( !m_pNodes )
...
AkDecisionTree::Node * AkDecisionTree::ResolvePathWeighted( AkArgumentValueID * in_pPath, AkUInt32 in_cPath, AkUniqueID in_idEvent, AkPlayingID in_idSequence, WeightedDecisionInfo& out_decisionInfo, AkCandidateCallbackFunc in_candidateCallbackFunc, void* in_pCookie  )
{
	AKASSERT( in_pPath );
	...
	_ResolvePathWeighted( m_pNodes, in_pPath, in_cPath, candidates, in_idEvent, in_candidateCallbackFunc, in_pCookie );
...
void AkDecisionTree::_ResolvePathWeighted( Node * in_pRootNode, AkArgumentValueID * in_pPath, AkUInt32 in_cPath, WeightedCandidates & io_candidates, AkUniqueID in_idEvent, AkCandidateCallbackFunc in_candidateCallbackFunc, void* in_pCookie )
{
	if( m_pNodes )
	{
		...
		if ( pNode ) 
		{
			if ( in_cPath == 1 )
			{
				if (!in_candidateCallbackFunc || in_candidateCallbackFunc(in_idEvent, pNode->audioNodeID, in_pCookie))
					AddCandidate( pNode, io_candidates );
			}
			else
			{
				_ResolvePathWeighted( pNode, in_pPath + 1, in_cPath - 1, io_candidates, in_idEvent, in_candidateCallbackFunc, in_pCookie );
			}
		}
			...
			&& *in_pPath != AK_FALLBACK_ARGUMENTVALUE_ID )
		{
			if ( in_cPath == 1 )
			{
				if (!in_candidateCallbackFunc || in_candidateCallbackFunc(in_idEvent, pChildrenNodes[0].audioNodeID, in_pCookie))
					AddCandidate(pChildrenNodes + 0, io_candidates);
			}
			else
			{
				_ResolvePathWeighted( pChildrenNodes, in_pPath + 1, in_cPath - 1, io_candidates, in_idEvent, in_candidateCallbackFunc, in_pCookie );
			}
		}
	}
}

AkUniqueID AkDecisionTree::ResolvePath( AkUniqueID in_idEvent, AkArgumentValueID * in_pPath, AkUInt32 in_cPath, AkPlayingID in_idSequence, AkCandidateCallbackFunc in_candidateCallbackFunc, void* in_pCookie )
{
	...
	{
		pResolved = _ResolvePath( m_pNodes, in_pPath, in_cPath, in_idEvent, in_candidateCallbackFunc, in_pCookie );
	}
	else
	{
		AKASSERT( m_uMode == Mode_Weighted );

		pResolved = ResolvePathWeighted( in_pPath, in_cPath, in_idEvent, in_idSequence, weightedDecisionInfo, in_candidateCallbackFunc, in_pCookie );
	}
...
AkDecisionTree::Node * AkDecisionTree::_ResolvePath( Node * in_pRootNode, AkArgumentValueID * in_pPath, AkUInt32 in_cPath, AkUniqueID in_idEvent, AkCandidateCallbackFunc in_candidateCallbackFunc, void* in_pCookie )
{
	if( m_pNodes )
	{
		Node * pChildrenNodes = m_pNodes + in_pRootNode->children.uIdx;
		...
		if ( pNode ) 
		{
			...
			Node * pResolved = _ResolvePath( pNode, in_pPath + 1, in_cPath - 1, in_idEvent, in_candidateCallbackFunc, in_pCookie );
			if ( pResolved )
				return pResolved;
		}
			...
			&& *in_pPath != AK_FALLBACK_ARGUMENTVALUE_ID )
		{
			if ( in_cPath == 1 )
			{
				...
				if ( !in_candidateCallbackFunc || in_candidateCallbackFunc( in_idEvent, pChildrenNodes->audioNodeID, in_pCookie ) )
					return pChildrenNodes + 0;
				else
					return NULL;
			}

			Node * pResolved = _ResolvePath( pChildrenNodes, in_pPath + 1, in_cPath - 1, in_idEvent, in_candidateCallbackFunc, in_pCookie );
			if ( pResolved )
				return pResolved;
		}
	}
