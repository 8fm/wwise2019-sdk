...
public:
	...
	AkReal32 TransitionTime( const AkRTPCKey& in_rtpcKey );
	...
	AKRESULT SerializeHistory(
		AK::IWriteBytes * in_pBytes
		);

	AKRESULT DeserializeHistory(
...
public:
	...
	void SafeResetSpecificInfo(CAkRegisteredObj * in_pGameObj);
...
private:
	...
	AKRESULT _PlayTrigger( AkPBIParams& in_rPBIParams );

	// Helper
	CAkContinueListItem * AddNewContListItem(AkPBIParams& in_rPBIParams);
