...
CAkAction* CAkAction::Create(AkActionType in_eActionType, AkUniqueID in_ulID)
{
	...
	case ACTION_TYPE_PLAYEVENT:
		return CAkActionPlayEvent::Create(in_eActionType, in_ulID);
	...
	case ACTION_TYPE_RESETHPF:
		return CAkActionSetAkProp::Create(in_eActionType, in_ulID, AkPropID_HPF);
	...
	case  ACTION_TYPE_RELEASE:
		return CAkActionRelease::Create(in_eActionType, in_ulID);

	case ACTION_TYPE_RESET_PLAYLIST:
		return CAkActionResetPlaylist::Create(in_eActionType, in_ulID);
...
void CAkAction::RemoveFromIndex()
{
	...
	if( ID() != AK_INVALID_UNIQUE_ID ) // Check if the action has an ID, it does not have an ID if was created by the sound engine
	{
		...
		key = AK_INVALID_UNIQUE_ID;// This is required because Wwise MAy disqualify actions that may still be referenced in game and that should not anymore be in the index.
	}
}
...
{
	AKASSERT( (ACTION_TYPE_ACTION & m_eActionType) == (in_ActionType & ACTION_TYPE_ACTION) );
...
AKRESULT CAkAction::SetInitialValues(AkUInt8* in_pData, AkUInt32 in_ulDataSize)
{
	...
	if (m_eActionType != ulActionType)
		return AK_Success; // Detected Change Type, happened because of a project mismatch, don't try to fix references in this case, we keep what we had from the proxy.
...
void CAkAction::SetPlayDirectly( bool in_bPlayDirectly )
{
	m_bPlayDirectly = in_bPlayDirectly;
}

bool CAkAction::PlayDirectly()
{
	return m_bPlayDirectly;
}
