...
AKRESULT CAkPlayingMgr::Init()
{
	...
	return m_PlayingMap.Init(AK_HASH_SIZE_VERY_SMALL) ? AK_Success : AK_Fail;
}
									 ...
									 AkUniqueID		in_id )
{
	...
	PlayingMgrItem* pItem = AkNew(AkMemID_Object, PlayingMgrItem());
	if (!pItem)
		return AK_Fail;
...
void CAkPlayingMgr::CancelCallbackGameObject( AkGameObjectID in_gameObjectID )
{
	m_csMapLock.Lock();
	for (AkPlayingMap::Iterator iter = m_PlayingMap.Begin(); iter != m_PlayingMap.End(); ++iter)
	{
		if (iter.pItem->GameObj == in_gameObjectID)
		{
			iter.pItem->pfnCallback = NULL;
			iter.pItem->uiRegisteredNotif &= ~(AK_CallbackBits); // remove callback bits if no callback
		}
...
void CAkPlayingMgr::CheckRemovePlayingID( AkPlayingID in_PlayingID, PlayingMgrItem* in_pItem )
{
	...
	if( in_pItem->cPBI == 0 && in_pItem->cAction == 0 )
	{
		...
		{
			g_pPositionRepository->VerifyNoPlayingID( in_PlayingID );
		}
#endif

		if( in_pItem->uiRegisteredNotif & AK_EnableGetSourceStreamBuffering )
		{
			g_pPositionRepository->RemovePlayingIDForBuffering( in_PlayingID );
		}
		...
		{
			AkRTPCKey rtpcKey;
			rtpcKey.PlayingID() = in_pItem->userParam.PlayingID();
			CAkRegisteredObj* pGameObj = g_pRegistryMgr->GetObjAndAddref(in_pItem->GameObj);
			if (pGameObj)
			{
			...
			in_pItem->Term(rtpcKey);
		}

		AkDelete(AkMemID_Object, in_pItem);
		...
		)
{
	...
	if ( pItem && pItem->uiRegisteredNotif & AK_Duration )
	{
		...
		info.mediaID = in_idMedia;
		info.bStreaming = in_bStreaming;
...
void CAkPlayingMgr::NotifyMarkers( AkMarkerNotification * pMarkers, AkUInt32 uNumMarkers )
{
	for( AkUInt32 i = 0; i < uNumMarkers; i++ )
	{
		NotifyMarker(pMarkers[i].pCtx, &pMarkers[i].marker);
	}
}

void CAkPlayingMgr::NotifyMusic(AkPlayingID in_PlayingID, AkCallbackType in_NotifType, const AkSegmentInfo & in_segmentInfo)
{
	m_csMapLock.Lock();
	PlayingMgrItem* pItem = m_PlayingMap.Exists( in_PlayingID );
	AKASSERT( pItem );
		...
		pItem->uiRegisteredNotif & in_NotifType )
	{
		...
		PrepareMusicNotification(in_PlayingID, pItem, in_NotifType, in_segmentInfo, NULL, info);
...
void CAkPlayingMgr::NotifyMusicUserCues(AkPlayingID in_PlayingID, const AkSegmentInfo & in_segmentInfo, char * in_pszUserCueName)
{
	m_csMapLock.Lock();
	PlayingMgrItem* pItem = m_PlayingMap.Exists( in_PlayingID );
	AKASSERT( pItem );
	...
	{
		AkMusicSyncCallbackInfo info;
		PrepareMusicNotification(in_PlayingID, pItem, AK_MusicSyncUserCue, in_segmentInfo, in_pszUserCueName,  info);
...
void CAkPlayingMgr::PrepareMusicNotification(AkPlayingID in_PlayingID, PlayingMgrItem* in_pItem, AkCallbackType in_NotifType, const AkSegmentInfo & in_segmentInfo, char * in_pszUserCueName, AkMusicSyncCallbackInfo & out_info)
{
	out_info.pCookie = in_pItem->pCookie;
	out_info.gameObjID = in_pItem->GameObj;

	out_info.playingID = in_PlayingID;
	out_info.segmentInfo = in_segmentInfo;
	out_info.musicSyncType = in_NotifType;

	out_info.pszUserCueName = in_pszUserCueName;
}

void CAkPlayingMgr::NotifyStarvation( AkPlayingID in_PlayingID, AkUniqueID in_idAudioNode )
{
	m_csMapLock.Lock();
	PlayingMgrItem* pItem = m_PlayingMap.Exists( in_PlayingID );
	if( pItem && 
		pItem->pfnCallback &&
		pItem->uiRegisteredNotif & AK_Starvation )
	{
		AkEventCallbackInfo info;
...
void CAkPlayingMgr::NotifyMidiEvent( AkPlayingID in_PlayingID, const AkMidiEventEx& in_midiEvent )
{
	m_csMapLock.Lock();
	PlayingMgrItem* pItem = m_PlayingMap.Exists( in_PlayingID );
	if( pItem && 
		pItem->pfnCallback &&
		pItem->uiRegisteredNotif & AK_MIDIEvent )
	{
		...
		info.midiEvent = in_midiEvent;

		AkCallbackFunc pFunc = pItem->pfnCallback;
		m_CallbackEvent.Reset();
		m_csMapLock.Unlock();
		(*pFunc)( AK_MIDIEvent, &info );
		m_CallbackEvent.Signal();
	}
	else
		m_csMapLock.Unlock();
...
bool CAkPlayingMgr::AddedNewRtpcValue( AkPlayingID in_PlayingID, AkRtpcID in_rtpcId )
{
	bool success = false;
	m_csMapLock.Lock();
	PlayingMgrItem* pItem = m_PlayingMap.Exists( in_PlayingID );
	if ( pItem )
	{
		success = pItem->AddedNewRtpcValue(in_rtpcId);
	}
	m_csMapLock.Unlock();
	return success;
}

bool CAkPlayingMgr::AddedNewModulatorCtx( AkPlayingID in_PlayingID, AkRtpcID in_rtpcId )
{
	bool success = false;
	m_csMapLock.Lock();
	PlayingMgrItem* pItem = m_PlayingMap.Exists( in_PlayingID );
	if ( pItem )
	{
		success = pItem->AddedNewModulatorCtx(in_rtpcId);
	}
	m_csMapLock.Unlock();
	return success;
}
...
AKRESULT CAkPlayingMgr::SetPBI( AkPlayingID in_PlayingID, CAkTransportAware* in_pPBI, AkUInt32 * out_puRegisteredNotif )
{
	...
	if(in_pPBI != NULL) // temp fix for WG-30087
	{
				...
				if( l_spContList )
				{
					...
					if( eResult == AK_Success )
					{
						...
						if( pAction )
						{
							...
							AkPendingAction* pPendingAction2 = AkNew(AkMemID_Object, AkPendingAction(in_GameObjPtr));
							if( pPendingAction2 )
							{
								...
								pAction->SetPlayStopTransition( pActionPAC->GetPlayStopTransition(), pPendingAction2 );
								pAction->SetPauseResumeTransition( pActionPAC->GetPauseResumeTransition(), pPendingAction2 );
