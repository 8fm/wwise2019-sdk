...
typedef AkSortedKeyArray<AkUniqueID, CAkParameterNodeBase*, AkHybridAllocator< sizeof( AkUniqueID ) >, AkChildIDValueGetKey> AkMapChildID;
...
public:
	...
	AKRESULT SetChildren( AkUInt8*& io_rpData, AkUInt32& io_rulDataSize )
	{
		...
		if( ulNumChilds )
		{
			...
			for( AkUInt32 i = 0; i < ulNumChilds; ++i )
			{
				...
				if( eResult != AK_Success && eResult != AK_IDNotFound)
				{
					break;
				}
			}
	...
	virtual void GatherSounds( AkSoundArray& io_aActiveSounds, AkSoundArray& io_aInactiveSounds, AkGameSyncArray& io_aGameSyncs, bool in_bIsActive, CAkRegisteredObj* in_pGameObj, AkUInt32 in_uUpdateGameSync=0, AkUInt32 in_uNewGameSyncValue=0   )
	{
		...
		for( ; iter != m_mapChildId.End(); ++iter )
		{
			(*iter)->GatherSounds( io_aActiveSounds, io_aInactiveSounds, io_aGameSyncs, in_bIsActive, in_pGameObj, in_uUpdateGameSync, in_uNewGameSyncValue );
		}
	}
