...
#include <AK/SoundEngine/Common/AkQueryParameters.h>  // for AkObjectInfo
...
CAkEvent::~CAkEvent()
{
	UnpinAllFilesInStreamCache(this);
...
{
	CAkEvent* pEvent = AkNew(AkMemID_Event, CAkEvent(in_ulID));
	if( pEvent )
	{
		pEvent->AddToIndex();
	}
...
{
	return AkNew(AkMemID_Event, CAkEvent(in_ulID));
}
...
AKRESULT CAkEvent::SetInitialValues( AkUInt8* in_pData, AkUInt32 in_ulDataSize)
{
	...
	AkUInt32 ulActionListSize = READVARIABLESIZEBANKDATA(AkUInt32, in_pData, in_ulDataSize );
...
AKRESULT CAkEvent::GatherSounds( AkSoundArray& out_aActiveSources, AkSoundArray& out_aInactiveSources, AkGameSyncArray& out_aGameSyncs, CAkRegisteredObj* in_pObj, AkUInt32 in_uUpdateGameSync, AkUInt32 in_uNewGameSyncValue )
{
	CAkEvent::AkActionList::Iterator iter = m_actions.Begin();
	while( iter != m_actions.End() )
	{
		...
		pObj->GatherSounds( out_aActiveSources, out_aInactiveSources, out_aGameSyncs, true, in_pObj, in_uUpdateGameSync, in_uNewGameSyncValue );
		pObj->Release();
...
void CAkEvent::SetPlayDirectly( bool in_bPlayDirectly )
{
	for (AkActionList::Iterator it = m_actions.Begin(); it != m_actions.End(); ++it)
	{
		(*it)->SetPlayDirectly(in_bPlayDirectly);
	}
}