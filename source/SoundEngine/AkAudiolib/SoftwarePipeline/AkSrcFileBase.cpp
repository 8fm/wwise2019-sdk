...
CAkSrcFileBase::CAkSrcFileBase( CAkPBI * in_pCtx )
	...
	, m_bIsMemoryStream( false )
	, m_bNextFetchWillLoop( false )
{
}
...
AKRESULT CAkSrcFileBase::_StartStream( AkAutoStmBufSettings & in_bufSettings )
{
	if ( m_bFormatReady )
	{
	...
	AKRESULT eResult = CreateStream( in_bufSettings, 0 );
...
AKRESULT CAkSrcFileBase::StartStream( AkUInt8 *, AkUInt32 )
{
	...
	return _StartStream(bufSettings);
}
...
{
	if (m_bIsMemoryStream || m_bIsReadingPrefecth)
	{
		m_pNextAddress = m_pNextAddress + (AkUIntPtr)in_pNewMedia - (AkUIntPtr)in_pOldMedia;
	}
	if (m_bIsMemoryStream)
	{
		return AK::IAkStreamMgr::Get()->RelocateMemoryStream(m_pStream, in_pNewMedia);
	}
	return AK_Success;
}
...
{
	return m_pAnalysisData != NULL && m_bHeaderFromBank;
}
	...
	)
{
	...
	// Create stream.
	AKRESULT eResult;
	m_bIsMemoryStream = pSrcType->mediaInfo.Type == SrcTypeMemory;
	if (m_bIsMemoryStream)
	{
		AkUInt8 * pBuffer;
		AkUInt32 uSize;
		m_pCtx->GetDataPtr( pBuffer, uSize );
	...
	if ( pFileName != NULL )
	{
		...
		if ( len < AK_MONITOR_STREAMNAME_MAXLENGTH )
			m_pStream->SetStreamName( pFileName );
		else
		{
...
AkFileSystemFlags CAkSrcFileBase::CreateFileSystemFlags(AkSrcTypeInfo * pSrcType)
{
...
{
	CAkSrcBaseEx::StopLooping();
	m_uStreamLoopCnt = 1;

	if (Stream_NextFetchWillLoop())
	{
		// Stream was going to loop; its position was already set back to m_ulLoopStart, which is wrong.
		if (m_bIsMemoryStream && m_pStream)
		{
			// For memory streams, we can 'repair' the state because we know stream seeking is instantaneous.
			if ( AK_EXPECT_FALSE( SetStreamPosition( m_ulLoopEnd ) != AK_Success ) )
				return AK_Fail;

			m_bIsLastStmBuffer = m_ulLoopEnd == m_uDataOffset + m_uDataSize;
			m_bNextFetchWillLoop = false;
		}
		else
		{
			// For file streams, too bad; we are going to loop an extra time.
			SetLoopCnt(2);
		}
	}
...
AKRESULT CAkSrcFileBase::FetchStreamBuffer()
{
    ...
    AKRESULT eResult = m_pStream->GetBuffer( 
		...
		uSize,       // Size of granted data space.
	...
	)
{
    ...
    if ( m_ulFileOffset >= ulEndLimit )
    {
		...
		if ( Stream_Doloop() )
		{
			...
			m_uStreamLoopCnt = m_uStreamLoopCnt == 0 ? 0 : m_uStreamLoopCnt - 1;
			m_bNextFetchWillLoop = true;
...
AKRESULT CAkSrcFileBase::StoreAnalysisData( AkFileParser::AnalysisDataChunk & in_analysisDataChunk )
{
	...
	{	
		m_pAnalysisData = (AkFileParser::AnalysisData*)AkAlloc( AkMemID_Processing, in_analysisDataChunk.uDataSize );
		if ( !m_pAnalysisData )
