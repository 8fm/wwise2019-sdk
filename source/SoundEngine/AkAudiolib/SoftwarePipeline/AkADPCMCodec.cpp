...
const AkInt32 CAkADPCMCodec::m_asNextStep[16] =
{
    -1, -1, -1, -1, 2, 4, 6, 8,
    -1, -1, -1, -1, 2, 4, 6, 8
};
...
const AkInt32 CAkADPCMCodec::m_asStep[NUMSTEPINDEXVAL] =
{
        7,     8,     9,    10,    11,    12,    13,
...
{
    if(AK_WAVE_FORMAT_ADPCM != wfx.wFormatTag)
    {
        return false;
    }
    if(wfx.nChannels < 1)
							...
							AkUInt32 nChannels )
{
    ...
    while(cBlocks--)
    {
        ...
        *psDst = (AkInt16)nPredSample;
        ...
        nStepSize = m_asStep[nStepIndex];
        nPredSample = DecodeSample(nEncSample, nPredSample, nStepSize);
        NextStepIndex(nEncSample, nStepIndex);
        *psDst = (AkInt16)nPredSample;
		psDst += nChannels;
