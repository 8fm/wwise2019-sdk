...
static inline void AbsoluteToRelativeSourceOffset(
...
{
	m_uLoopCnt = m_pCtx ? m_pCtx->GetLooping() : LOOPING_ONE_SHOT;
}
...
AKRESULT CAkSrcBaseEx::Seek( AkUInt32 in_SourceOffset )
{
...
// Returns sample rate of the source for the purpose of seeking.
AkUInt32 CAkSrcBaseEx::GetSourceSampleRate() const
{
	// Default implementation returns the sample rate of the source stored in the PBI.
	return m_pCtx->GetMediaFormat().uSampleRate;
}
...
{	 
	...
	return ( fTotalNumSamples * 1000.f ) / (AkReal32)GetSourceSampleRate();		// mSec.
}

AkReal32 CAkSrcBaseEx::GetDurationNoLoop() const
{
    return ( m_uTotalSamples * 1000.f ) / (AkReal32)GetSourceSampleRate();	// mSec.
}
		...
		do
		{
			while ( uNextEntry < uNumEntries )
			{
					...
					&& uCurSample < uNextPosition )
				{
					...
					// Normalize.
					AKASSERT(-fAttenuation <= m_pAnalysisData->fEnvelopePeak);
			...
			if ( uCurSample >= pPoint->uPosition )
			{
				...
				// Normalize.
				AKASSERT(-(AkReal32)pPoint->uAttenuation <= m_pAnalysisData->fEnvelopePeak);
	...
	)
{
	...
	io_state.AttachInterleavedData( in_pData, in_uNumSamplesProduced, in_uNumSamplesProduced, in_channelConfig );

	// Markers and position info.
	SaveMarkersForBuffer( io_state, m_uCurSample, io_state.uValidFrames );
...
void CAkSrcBaseEx::SaveMarkersForBuffer( 
...
AkUInt32 CAkSrcBaseEx::GetSourceOffset()
{
	...
	if ( m_pCtx->IsSeekRelativeToDuration() )
	{
		...
		uAbsoluteSourceOffset = m_pCtx->GetSeekPosition( GetSourceSampleRate(), uSourceDuration, bSnapToMarker );
	}
	else
	{
		uAbsoluteSourceOffset = m_pCtx->GetSeekPosition( GetSourceSampleRate(), bSnapToMarker );
	}
...
void CAkSrcBaseEx::ForceVirtualize()
{
	m_pCtx->VirtualizeForInterruption();
}

void CAkSrcBaseEx::ForceDevirtualize()
{
	m_pCtx->DevirtualizeForInterruption();
}