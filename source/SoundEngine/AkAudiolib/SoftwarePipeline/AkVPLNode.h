...
//	virtual void		ConsumeBuffer( AkVPLState & io_state ) {}		// give data to node
	...
	virtual AkReal32	GetPitch() const { return ( m_pInput ) ? m_pInput->GetPitch() : 0.f; }

	virtual void		PopMarkers(AkUInt32 uNumMarkers) { if (m_pInput) m_pInput->PopMarkers(uNumMarkers); }

	virtual AkChannelMappingFunc GetChannelMappingFunc() { return m_pInput ? m_pInput->GetChannelMappingFunc() : nullptr; }
