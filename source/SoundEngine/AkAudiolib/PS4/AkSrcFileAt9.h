...
public:
				...
				)
	{
		...
		if (in_uStitchBufferFillSize > 0)
		{
			if (m_pStitchBuffer == NULL)
			{
				...
				m_pStitchBuffer = (AkUInt8*) AkAlloc( AkMemID_Processing, in_nBlockSize * sizeof(AkUInt8) );
	...
	bool IsStitchBufferUsedUp() const
	{ 
		return m_uStitchBufferFillSize == 0 || ((m_uDataLeft + m_uStitchBufferFillSize) - m_uInDecoderBytes) == 0;
	}
	...
	AkUInt8* GetAddress() const { AKASSERT(m_pBuffer != NULL); return m_pBuffer; }
...
//		AKPLATFORM::OutputDebugMsg( msg );
		...
		{
			AKASSERT((m_uInDecoderBytes - m_uStitchBufferFillSize) <= m_uDataLeft);
	...
	{
#ifdef AT9_STREAM_DEBUG
		AKASSERT((m_uInDecoderBytes + in_length) <= ( m_uDataLeft + m_uStitchBufferFillSize));
...
public:
	...
	AkUInt32							StoredSamplesCount() const 
										{ 
											// stich buffer might not be a complete block, floor the result
											return ByteToSampleFloor(m_pACPStreamBuffer[0].GetSizeLeft() + m_pACPStreamBuffer[1].GetSizeLeft());
										}
	...
	virtual bool						IsLoopOnThisFrame( AkUInt32 in_numSampleToRead ) const
										{		
											...
											AkInt32 lastSample = (AkInt32)m_DecoderSamplePosition + (AkInt32)AkMin(in_numSampleToRead, StoredSamplesCount());
											return (lastSample > (AkInt32)this->m_uPCMLoopEnd && Decoder_Doloop());
										}
